---
title: Compliance
description: How Granit enforces GDPR, ISO 27001, and ISO 9001 through architectural constraints — not afterthoughts
sidebar:
  order: 9
---
...
- **Granit.Vault** -- HashiCorp Vault Transit engine. The encryption key never leaves Vault. Required for production under ISO 27001.
...
`Granit.Observability` configures OpenTelemetry with Serilog structured logging, distributed traces, and metrics. A single HTTP request is traceable through all async handlers, background jobs, and external service calls via the W3C Trace Context propagation.

Every log entry includes:

- `TraceId` and `SpanId` for correlation
- `TenantId` for tenant-scoped filtering
- `UserId` for actor attribution

### Document versioning

The Templating module assigns a `RevisionId` to every template version. When a document is generated, the `RevisionId` propagates into the output metadata. This satisfies ISO 9001's requirement for document version control -- you can always answer "which template version produced this PDF?"

### Workflow -- audited state transitions

The Workflow module provides a finite state machine engine where every transition is validated and logged:

1. The FSM validates that the transition is allowed from the current state.
2. Transition guards (business rules) are evaluated.
3. The state change is persisted in a single transaction with the entity update.
4. A Timeline entry records who triggered the transition, when, and from which state to which state.

No state change happens without a trace.
...
| Module | GDPR Art. 17 | GDPR Art. 18 | GDPR Art. 25 | ISO 27001 Audit | ISO 27001 Encryption | ISO 9001 Traceability |
|--------|:---:|:---:|:---:|:---:|:---:|:---:|
...
| Granit.Vault | -- | -- | Pseudonymization | -- | Transit engine | -- |
...
| Granit.Observability | -- | -- | -- | Structured logs | -- | Traces + metrics |
| Granit.Timeline | -- | -- | -- | Activity log | -- | Event history |
| Granit.Workflow | -- | -- | -- | Transition log | -- | State audit |
| Granit.Templating | -- | -- | -- | -- | -- | Revision tracking |
...
- [Privacy reference](/reference/modules/privacy/) -- full API surface of `Granit.Privacy`
- [Persistence reference](/reference/modules/persistence/) -- `AuditedEntity`, interceptors, query filters
