...
description: JWT authentication, Keycloak and Entra ID integration, RBAC authorization with per-role caching, and back-channel logout
sidebar:
  order: 8
---
...
- **Pluggable authentication** -- swap Keycloak for Entra ID without touching application code.
...
The authentication pipeline is built from four packages, each adding one concern on top of the previous one.

```mermaid
graph TD
    S["Granit.Security\nICurrentUserService abstraction"] --> C[Granit.Core]
    JWT["Granit.Authentication.JwtBearer\nGeneric OIDC JWT Bearer"] --> S
    KC["Granit.Authentication.Keycloak\nKeycloak claims transformation"] --> JWT
    EID["Granit.Authentication.EntraId\nEntra ID roles parsing"] --> JWT
```
...
sequenceDiagram
    ...
    CT->>CT: Extract roles (Keycloak/EntraID/generic)
...
- [Security reference](/reference/modules/security/) -- full API surface, configuration tables, EF Core permission store
- [Compliance concept](./compliance/) -- how the security model supports GDPR and ISO 27001
- [Identity reference](/reference/modules/identity/) -- user management and Keycloak Admin API
