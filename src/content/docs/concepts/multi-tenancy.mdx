...
Two built-in resolvers run in priority order — **first match wins**:
...
```mermaid
flowchart TD
    A[New tenant onboarding] --> B{"ISO 27001 physical\nseparation required?"}
    B -- Yes --> C[DatabasePerTenant]
    B -- No --> D{"More than 1000\ntenants expected?"}
...
in `Granit.Core.Domain` enforces this. Non-nullable `Guid` will fail the
architecture tests.
:::
...
- [Multi-Tenancy reference](/reference/modules/multi-tenancy/) — configuration,
  API surface, and extension methods
