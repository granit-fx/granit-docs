...
description: Protect your API with JWT Bearer authentication and Keycloak claims transformation
sidebar:
  order: 3
---
...
:::note[Good to know]
`GranitAuthenticationKeycloakModule` transitively depends on `GranitJwtBearerModule` — you do not need to declare both. The module system resolves the full dependency graph for you.
:::
...
- **JWT validation** — `GranitJwtBearerModule` reads the `JwtBearer` configuration section and sets up token validation parameters (issuer, audience, signing keys).
...
See the [Observability reference](/reference/modules/observability/) for the full configuration guide.
...
[Project Templates](/getting-started/project-templates/)
