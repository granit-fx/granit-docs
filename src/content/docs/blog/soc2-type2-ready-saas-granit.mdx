...
tags:
  ...
  - compliance
  - deep-dive
description: "Enterprise buyers want SOC 2 Type 2 before signing. How Granit's modules map to the five Trust Service Criteria — and what the framework cannot replace."
excerpt: >
...
- **Metrics**: every module emits named metrics via `IMeterFactory` (e.g. `granit.authorization.check.denied`, `granit.persistence.cross_tenant_query`). Mimir stores and alerts on them.
...
```csharp title="PatientPrivacyDataProvider.cs"
    ...
    IStagedFragmentBuilder fragmentBuilder) : IPrivacyDataProvider
{
    ...
    public ValueTask<bool> HasDataAsync(
    ...
    {
        // Streamed and aggregated by the Privacy module for portability requests
    }
}
```
...
| P4.1 — Personal data portability | `IPrivacyDataProvider` aggregation | `Granit.Privacy` |
