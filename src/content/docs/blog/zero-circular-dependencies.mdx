---
title: "376 Packages, Zero Circular Dependencies"
date: 2026-03-11
sidebar:
  order: 9
authors:
...
tags:
  - architecture
  - code-quality
description: "Granit ships 376 NuGet packages with zero circular dependencies, enforced by Kahn's algorithm at startup and architecture tests at build time."
excerpt: >
  Granit ships 376 NuGet packages with zero circular dependencies, enforced by
  Kahn's algorithm at startup and architecture tests at build time.
---

A framework with 376 NuGet packages sounds like a dependency management nightmare. At that scale, one careless `ProjectReference` can create a cycle that makes incremental builds impossible, forces unnecessary recompilation, and turns your dependency graph into a tangled mess that no one dares to refactor.
...
**Incremental builds are fast.** When a developer changes `Granit.Notifications.Email.Smtp`, only that project and its dependents rebuild. There are no hidden reverse edges that pull in unrelated packages. On a 376-package solution, this is the difference between a 4-second incremental build and a 40-second full rebuild.
...
The combination of runtime DAG validation, build-time architecture tests, and a layered design that makes cycles unlikely creates a system where the dependency graph is both a design artifact and an enforced invariant. At 376 packages, that enforcement is not optional — it is the only thing standing between a modular framework and an accidental monolith.
