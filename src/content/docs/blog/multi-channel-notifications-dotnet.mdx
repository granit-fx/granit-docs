---
title: "Multi-Channel Notifications in .NET"
date: 2026-05-01
authors:
  - jfmeyers
tags:
  ...
  - real-time
  - architecture
description: "One PublishAsync call, nine channels. How Granit fans out notifications to email, SMS, WhatsApp, push, SignalR, SSE and more without touching your domain."
excerpt: >
  One PublishAsync call, nine delivery channels. How Granit fans out a single
  notification to email, SMS, WhatsApp, mobile push, web push, SignalR, SSE,
  Zulip, and the in-app inbox without rewriting your domain code.
---
...
| Zulip | `NotificationChannels.Zulip` | Zulip REST API (team chat) |
...
## Publishing — one call, every eligible channel
...
7. Writes one `NotificationDelivery` row per attempted channel — with status, timestamp, error message, and attempt number.
...
`PreferredCulture` is the hook that makes localization automatic — Granit ships templates in 18 cultures and selects the right one per recipient.
...
- **Localization.** 18 cultures (15 base + 3 regional). Templates ship as `.html` / `.fr.html` / `.de.html`. Recipients with `PreferredCulture = "fr-CA"` get the closest match.
