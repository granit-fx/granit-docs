...
tags:
  ...
  - release
  - security
description: "Turn any Granit module into an MCP server in 3 steps. Claude, Copilot and Cursor discover and invoke your tools — with RBAC, GDPR scrubbing, multi-tenancy."
excerpt: >
  The new Granit.Tools.Mcp module family wraps the MCP C# SDK to expose your application
...
Your API already exposes endpoints for humans. AI agents are its next consumers —
and the usual shortcuts give them either too little access (a hand-written chatbot
proxy that covers four endpoints) or far too much (an API key with god-mode scope).
What agents actually need is your existing authorization, your multi-tenancy
isolation, and automatic PII redaction.

That's what the **Model Context Protocol** (MCP) enables. And with the new
`Granit.Tools.Mcp` module family, any Granit module can become an MCP server in 3 steps.
...
| Concern | MCP SDK alone | With Granit.Tools.Mcp |
|---------|--------------|-----------------|
...
flowchart LR
    subgraph app["Your Granit Application"]
        direction TB
        T1["Invoice tools"] --> MCP["Granit.Tools.Mcp\nAuto-discovery\n+ Filters"]
        T2["Workflow tools"] --> MCP
        T3["BlobStorage tools"] --> MCP
        MCP --> SRV["Granit.Tools.Mcp.Server\nHTTP + Auth"]
    end
    ...
    CLI["Granit.Tools.Mcp.Client"] -- "HTTP" --> ERP
...
| Package | Purpose |
|---------|---------|
| **Granit.Tools.Mcp** | Auto-discovery, sanitization pipeline, metrics |
| **Granit.Tools.Mcp.Server** | ASP.NET Core HTTP transport, `[Authorize]`, permissions |
| **Granit.Tools.Mcp.Client** | Connect to external MCP servers (HTTP + stdio) |
...
```bash
dotnet add package Granit.Tools.Mcp.Server
```
...
The flow works both ways. `Granit.Tools.Mcp.Client` provides a named factory for connecting
...
- **Granit.Tools.Mcp wraps the official SDK** with auto-discovery, RBAC authorization,
