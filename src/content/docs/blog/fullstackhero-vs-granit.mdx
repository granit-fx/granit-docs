---
title: "FullStackHero vs Granit: Two .NET Frameworks"
date: 2026-04-03
sidebar:
  order: 17
authors:
...
| **NuGet packages** | ~15 internal projects (not published) | %%PACKAGE_COUNT%% published packages |
...
| **Localization** | Not built-in | 18 cultures, source-generated keys, runtime overrides |
| **Notifications** | MailKit/SendGrid | 9 channels with fan-out + preference filtering |
| **Documentation** | Mintlify + blog articles | Astro + Starlight (60 patterns, 57 ADRs) |
...
:::tip[Already running on FullStackHero?]
If you have already shipped on FullStackHero and the decision to move is
made, the
[FullStackHero → Granit migration guide](/migrating-from/fullstackhero/)
walks through the strangler-fig cutover, the Mediator → Wolverine port,
the Finbuckle → `Granit.MultiTenancy` switch, and the `AppDbContext` →
isolated `DbContext` refactor. This article is the *decision*; that guide
is the *execution*.
:::
...
**Granit is deeper.** %%PACKAGE_COUNT%% packages means more surface area to learn, but also more
...
- [Migrating from FullStackHero to Granit](/migrating-from/fullstackhero/) —
  the step-by-step migration guide (cutover model, before/after code, FAQ)
