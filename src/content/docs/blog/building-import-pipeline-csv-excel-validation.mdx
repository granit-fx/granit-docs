...
```csharp title="InvoiceImportDefinition.cs"
...
Add `Granit.DataExchange.Wolverine` and the dispatchers switch to the transactional outbox. The job survives a crash, is retried on transient failures, and lands in the dead-letter queue if it permanently fails. Same handlers, same `ImportDefinition<T>`, durable. We unpack the upgrade path in [From Channels to Wolverine](/blog/from-channels-to-wolverine-upgrading-messaging/).
...
* **Multi-tenancy on every entity** — `ImportJobEntity`, `SavedMappingEntity`, `ExternalIdMappingEntity` all implement `IMultiTenant`. The query filter from [the interceptor stack](/blog/soft-deletes-audit-trails-query-filters-interceptor-stack/) keeps tenants from seeing each other's import history.
