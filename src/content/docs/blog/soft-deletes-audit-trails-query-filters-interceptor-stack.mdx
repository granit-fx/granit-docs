...
| Metric | Tags | Use |
| --- | --- | --- |
| `granit.persistence.entity.purged` | `tenant_id` | Retention sweeps — how much soft-deleted data actually leaves the database |
| `granit.persistence.cross_tenant_query` | `entity_name`, `origin`, `tenant_id` | Every query that bypassed the tenant filter |

Plot both in Grafana. The first tells you whether your retention job is really
reclaiming rows. The second is the one to alert on: `origin` separates a
deliberate `QueryAcrossTenants` call from `implicit_unsignaled` — a bypass with
no host-access signal behind it, which is what a tenant-isolation bug looks
like in production.
