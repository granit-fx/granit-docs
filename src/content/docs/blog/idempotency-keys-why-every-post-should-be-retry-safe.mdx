...
| -------- | ------------------ |
| `granit.http.idempotency.lock.acquired` | Successful lock acquisitions (new requests) |
| `granit.http.idempotency.lock.conflict` | 409 responses (concurrent duplicates) |
| `granit.http.idempotency.response.replayed` | Cached responses served (successful dedup) |
| `granit.http.idempotency.hash.mismatch` | 422 responses (key reuse with different payload) |
