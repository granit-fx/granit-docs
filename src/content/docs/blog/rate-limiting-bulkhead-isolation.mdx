---
title: "Rate Limiting + Bulkhead Isolation: Protecting APIs From Themselves"
date: 2026-06-12
sidebar:
  order: 37
authors:
...
tags:
  ...
  - performance
  - multi-tenancy
description: "Rate limiting and bulkhead isolation solve two distinct failure modes. Learn when each pattern applies and how Granit wires both together with minimal configuration."
excerpt: >
...
|---|---|---|
| `granit.http.bulkhead.leases.active` | UpDownCounter | Currently active leases per policy and tenant |
| `granit.http.bulkhead.requests.rejected` | Counter | Rejected requests (bulkhead full) |

Both metrics carry `policy` and `tenant_id` attributes, so you can alert on a per-tenant surge without aggregating across the platform. A spike in `granit.http.bulkhead.requests.rejected` for a single tenant is the early warning signal that rate limiting alone would have hidden until the `429`s started.
