...
tags:
  ...
  - architecture
  - frontend
description: "Granit.Bff is a Backend For Frontend proxy that keeps OIDC tokens server-side. Your SPA only handles HttpOnly cookies — zero token exposure, auto refresh."
excerpt: >
...
Real-world platforms rarely have a single SPA. A platform built on Granit can serve four distinct frontends — Admin, Physician, Patient, and Insurer — each with different user populations, permissions, and OIDC scopes. All four share the same backend API and identity provider.
...
- [Granit.Bff documentation](/dotnet/security/bff/) — architecture, configuration, and getting started
- [Token management](/dotnet/security/token-management/) — how tokens are stored, refreshed, and revoked server-side
