...
tags:
  - release
  - architecture
description: "Granit is an open-source modular framework for .NET 10 — 93 packages, zero circular dependencies, GDPR and ISO 27001 compliance built in."
excerpt: >
  Granit is an open-source modular framework for .NET 10 — 93 packages, zero
...
- **Encryption at rest**: `Granit.Vault` integrates with HashiCorp Vault's Transit engine for field-level encryption. Sensitive data — national identification numbers, health records, financial details — is encrypted before it reaches the database.
...
<Tabs>
  <TabItem label="Core & Utilities">
    ...
    |---------|-------------|
    | `Granit.Core` | Module system (`GranitModule`, `[DependsOn]`), shared domain types (`Entity`, `AggregateRoot`, `ValueObject`) |
  ...
  <TabItem label="Security & Identity">
    ...
    | Package | What it does |
    |---------|-------------|
    | `Granit.Security` | `ICurrentUserService`, security abstractions |
    | `Granit.Authentication.JwtBearer` | JWT Bearer middleware with Keycloak token validation |
    | `Granit.Authentication.Keycloak` | Claims transformation for Keycloak tokens |
    | `Granit.Authorization` | Policy-based authorization with EF Core store |
    | `Granit.Vault` | HashiCorp Vault integration — Transit encryption, dynamic database credentials |
  ...
  <TabItem label="Data & Persistence">
    ...
    | `Granit.Caching` | Distributed caching with HybridCache and Redis support |
  ...
  <TabItem label="Messaging & Notifications">
    ...
    | `Granit.Notifications.Email` | Email via SMTP or Brevo |
    | `Granit.Notifications.Sms` | SMS delivery |
    | `Granit.Notifications.WhatsApp` | WhatsApp Business API |
    | `Granit.Notifications.Push` | Web Push and Mobile Push (FCM) |
...
Granit is released under the **Apache-2.0** license. The source code, documentation, architecture tests, and all 93 packages are open. We chose Apache-2.0 deliberately: it is permissive enough for commercial use while providing patent protection that MIT does not.
...
- **Additional identity providers** beyond Keycloak (Entra ID, Auth0)
...
- [Getting started](/getting-started/) — install Granit and build your first module
- [Modular monolith vs microservices](/concepts/modular-monolith-vs-microservices/) — the architecture spectrum and where Granit fits
...
- [Module reference](/reference/) — the complete package catalog with dependency graphs
