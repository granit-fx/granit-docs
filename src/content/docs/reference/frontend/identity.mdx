...
| Capability | Keycloak | Entra ID |
|-----------|----------|----------|
| Individual session termination | Yes | No |
| Native password reset email | Yes | Yes |
| Group hierarchy | Yes | Yes |
| Custom attributes | Yes (unlimited) | Yes (limited) |
| Credential verification | Yes (ROPC) | Varies |
...
- [Granit.Identity module](/reference/modules/security/) — .NET identity provider abstractions and Keycloak implementation
- [Authentication](./authentication/) — Keycloak session that provides the JWT for identity operations
