...
description: HashiCorp Vault and Azure Key Vault transit encryption, dynamic credentials, AES-256 string encryption
sidebar:
  order: 21
---
...
and dynamic credentials via Key Vault Secrets, and Granit.Vault.Aws targets AWS KMS and
Secrets Manager. All provider modules are automatically disabled in Development environments
...
|----------|-----------|---------|
| Modules | `GranitEncryptionModule`, `GranitVaultHashiCorpModule`, `GranitVaultAzureModule`, `GranitVaultAwsModule` | — |
...
| Extensions | `AddGranitEncryption()`, `AddGranitVaultHashiCorp()`, `AddGranitVaultAzure()`, `AddGranitVaultAws()` | — |
