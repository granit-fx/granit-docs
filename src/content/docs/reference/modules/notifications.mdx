...
| Email | `AddGranitNotificationsEmail()` | Keyed Service: `"Smtp"`, `"Brevo"` |
| SMS | `AddGranitNotificationsSms()` | Keyed Service: `"Brevo"` |
| WhatsApp | `AddGranitNotificationsWhatsApp()` | Keyed Service: `"Brevo"` |
| Mobile Push | `AddGranitNotificationsMobilePush()` | Keyed Service: `"Fcm"` |
...
|----------|-----------|-------|------|
| SMTP | `AddGranitSmtpCheck()` | EHLO handshake via MailKit | readiness |
| SES | `AddGranitSesCheck()` | `GetAccount()` — Degraded if sending paused | readiness, startup |
| Brevo | `AddGranitBrevoCheck()` | `GET /account` | readiness |

All checks sanitize error messages — credentials, hostnames, and API keys are never
exposed in the health check response.
...
| Options | `NotificationsOptions`, `EmailChannelOptions`, `SmtpOptions`, `BrevoOptions`, `SmsChannelOptions`, `MobilePushChannelOptions`, `FcmOptions`, `SignalRChannelOptions`, `PushChannelOptions`, `SseChannelOptions`, `ZulipChannelOptions`, `ZulipBotOptions` | various |
| Email | `IEmailSender`, `EmailMessage` | `Granit.Notifications.Email` |
...
| Extensions | `AddGranitNotifications()`, `AddGranitNotificationsEntityFrameworkCore()`, `AddGranitNotificationsEmail()`, `AddGranitNotificationsEmailSmtp()`, `AddGranitNotificationsBrevo()`, `AddGranitNotificationsSms()`, `AddGranitNotificationsWhatsApp()`, `AddGranitNotificationsMobilePush()`, `AddGranitNotificationsMobilePushFcm()`, `AddGranitNotificationsSignalR()`, `AddGranitNotificationsPush()`, `AddGranitNotificationsSse()`, `AddGranitNotificationsZulip()`, `AddNotificationDefinitions<T>()`, `MapGranitNotificationEndpoints()`, `MapMobilePushTokenEndpoints()` | various |
