...
Granit modules provide opt-in health checks via `AddGranit*Check()` extension methods
...
|--------|-----------------|-------|------|
| `Granit.Persistence` | `AddGranitDatabaseCheck()` | EF Core `CanConnectAsync` | readiness, startup |
| `Granit.Caching.StackExchangeRedis` | `AddGranitRedisCheck()` | Redis `PING` with latency threshold | readiness, startup |
| `Granit.Vault` | `AddGranitVaultCheck()` | Vault seal status + auth | readiness, startup |
| `Granit.Vault.Aws` | `AddGranitKmsCheck()` | KMS `DescribeKey` (Degraded on PendingDeletion) | readiness, startup |
| `Granit.Identity.Keycloak` | `AddGranitKeycloakCheck()` | `client_credentials` token request | readiness, startup |
| `Granit.Identity.EntraId` | `AddGranitEntraIdCheck()` | `client_credentials` token request | readiness, startup |
| `Granit.BlobStorage.S3` | `AddGranitS3Check()` | `ListObjectsV2(MaxKeys=1)` | readiness, startup |
| `Granit.Notifications.Email.Smtp` | `AddGranitSmtpCheck()` | EHLO handshake via MailKit | readiness |
| `Granit.Notifications.Email.Ses` | `AddGranitSesCheck()` | `GetAccount()` (Degraded if sending paused) | readiness, startup |
| `Granit.Notifications.Brevo` | `AddGranitBrevoCheck()` | `GET /account` | readiness |
...
builder.Services.AddHealthChecks()
    .AddGranitDatabaseCheck()
    .AddGranitRedisCheck(degradedThreshold: TimeSpan.FromMilliseconds(100))
    .AddGranitKeycloakCheck()
    .AddGranitSesCheck()
    .AddGranitBrevoCheck();
```
...
- [Notifications module](./notifications/) -- SMTP, SES, and Brevo health checks
