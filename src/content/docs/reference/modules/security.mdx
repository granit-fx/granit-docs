...
JWT Bearer validation, Keycloak claims transformation, dynamic RBAC permissions with
per-role caching, back-channel logout — all wired through the module system.
...
|----------|-----------|---------|
| Module | `GranitSecurityModule`, `GranitJwtBearerModule`, `GranitAuthenticationKeycloakModule`, `GranitAuthorizationModule`, `GranitAuthorizationEntityFrameworkCoreModule` | — |
...
| Authorization options | `GranitAuthorizationOptions`, `JwtBearerAuthOptions`, `KeycloakOptions` | — |
...
| Extensions | `AddGranitJwtBearer()`, `AddGranitKeycloak()`, `AddGranitAuthorization()`, `AddGranitAuthorizationEntityFrameworkCore<T>()`, `MapBackChannelLogout()` | — |

## See also

- [Identity module](./identity.mdx) — User management, Keycloak Admin API, user cache
- [Privacy module](./privacy.mdx) — GDPR compliance, cookies, CORS
- [Core module](./core.mdx) — `ICurrentTenant`, exception hierarchy
- [Persistence module](./persistence.mdx) — `AuditedEntity` base class, interceptors
