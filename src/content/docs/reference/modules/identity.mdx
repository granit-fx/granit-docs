...
description: Identity provider abstractions, Keycloak Admin API, Cognito User Pool API, user cache with GDPR support, REST endpoints
sidebar:
  order: 4
---
...
or any OIDC provider handles authentication — Granit.Identity handles everything else: user
...
|----------|-----------|---------|
| Module | `GranitIdentityModule`, `GranitIdentityKeycloakModule`, `GranitIdentityCognitoModule`, `GranitIdentityEntityFrameworkCoreModule`, `GranitIdentityEndpointsModule` | — |
...
| Extensions | `AddGranitIdentity()`, `AddIdentityProvider<T>()`, `AddGranitIdentityKeycloak()`, `AddGranitIdentityCognito()`, `AddGranitIdentityEntityFrameworkCore<T>()`, `AddGranitIdentityEndpoints()`, `MapIdentityUserCacheEndpoints()` | — |
