...
description: Identity provider abstractions, Keycloak Admin API, user cache with GDPR support, REST endpoints
sidebar:
  order: 4
---
...
Granit.Identity provides a provider-agnostic identity management layer. Keycloak (or any
OIDC provider) handles authentication — Granit.Identity handles everything else: user
...
```csharp
context.Services.AddIdentityProvider<EntraIdIdentityProvider>();
```
...
|----------|-----------|---------|
| Module | `GranitIdentityModule`, `GranitIdentityKeycloakModule`, `GranitIdentityEntityFrameworkCoreModule`, `GranitIdentityEndpointsModule` | — |
...
| Extensions | `AddGranitIdentity()`, `AddIdentityProvider<T>()`, `AddGranitIdentityKeycloak()`, `AddGranitIdentityEntityFrameworkCore<T>()`, `AddGranitIdentityEndpoints()`, `MapIdentityUserCacheEndpoints()` | — |

## See also

- [Security module](./security.mdx) — Authentication, authorization, RBAC permissions
- [Privacy module](./privacy.mdx) — GDPR data export/deletion, cookie consent
- [Persistence module](./persistence.mdx) — `AuditedEntity`, interceptors
