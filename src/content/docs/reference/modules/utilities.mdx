...
- [Core module](./core.mdx) — Exception hierarchy, `IHasValidationErrors`
- [Security module](./security.mdx) — `ICurrentUserService` (often used alongside `IClock` for audit)
- [Persistence module](./persistence.mdx) — `AuditedEntityInterceptor` uses `IClock` for timestamps
