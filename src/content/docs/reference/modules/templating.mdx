---
title: Granit.Templating & DocumentGeneration
...
flowchart LR
    ...
    subgraph "2. Resolve"
        ED --> R1["StoreTemplateResolver\n(Priority=100)"]
        R1 -->|miss| R2["EmbeddedTemplateResolver\n(Priority=-100)"]
        R1 -->|hit| TD[TemplateDescriptor]
        R2 --> TD
    end
    subgraph "3. Render"
        TD --> ENG["ITemplateEngine\n(Scriban)"]
        ED --> ENG
        GC["Global Contexts\nnow.*, context.*"] --> ENG
...
flowchart LR
    ...
    ENGINE -->|TextRenderedContent| RENDERER["IDocumentRenderer\n(PuppeteerSharp)"]
...
stateDiagram-v2
    ...
    Draft --> PendingReview : Workflow bridge\n(optional)
...
- [Core module](./core.mdx) -- Module system, domain base types
- [Persistence module](./persistence.mdx) -- `AuditedEntityInterceptor`, `ApplyGranitConventions`
- [Wolverine module](./wolverine.mdx) -- Durable messaging for async document generation
- [Notifications module](./notifications.mdx) -- Email, SMS, push channels that consume rendered templates
- [Background Jobs module](./background-jobs.mdx) -- Schedule document generation as background jobs
