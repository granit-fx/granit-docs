...
flowchart TD
    ...
    T1[Tier 1: Saved mappings<br/><em>Previously confirmed by user</em>] --> T2
    T2[Tier 2: Exact match<br/><em>Property name, display name, aliases</em>] --> T3
    T3[Tier 3: Fuzzy match<br/><em>Levenshtein distance >= threshold</em>] --> T4
    T4[Tier 4: Semantic / AI<br/><em>Header metadata only, GDPR-safe</em>] --> R[Suggested mappings]
```
...
flowchart LR
    ...
    B -- "<=threshold" --> C[Synchronous export]
...
- [Core module](./core.mdx) — Module system, domain types
- [Persistence module](./persistence.mdx) — EF Core interceptors, `ApplyGranitConventions`
- [Wolverine module](./wolverine.mdx) — Transactional outbox, context propagation
- [Background jobs module](./background-jobs.mdx) — Scheduling integration
