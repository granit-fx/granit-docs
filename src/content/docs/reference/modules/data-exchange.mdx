...
flowchart TD
    ...
    T1["Tier 1: Saved mappings\nPreviously confirmed by user"] --> T2
    T2["Tier 2: Exact match\nProperty name, display name, aliases"] --> T3
    T3["Tier 3: Fuzzy match\nLevenshtein distance >= threshold"] --> T4
    T4["Tier 4: Semantic / AI\nHeader metadata only, GDPR-safe"] --> R[Suggested mappings]
```
