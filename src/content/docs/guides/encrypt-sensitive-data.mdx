...
- `Granit.Vault` for HashiCorp Vault Transit encryption (production)
- A running HashiCorp Vault instance with Transit engine enabled (production only)
...
`Granit.Vault` depends on `Granit.Encryption` -- both are installed automatically.
...
| `VaultStringEncryptionProvider` | `"Vault"` | 10--20 ms | High-security operations, infrequent writes |
...
- [Manage application settings](/guides/manage-application-settings/) -- encrypted settings with cascading resolution
- [Vault and Encryption reference](/reference/modules/vault-encryption/) -- full API and configuration reference
- [Observability reference](/reference/modules/observability/) -- monitor encryption operations with OpenTelemetry
