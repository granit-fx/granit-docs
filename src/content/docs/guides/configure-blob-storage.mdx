...
description: Set up S3-compatible file storage with presigned URLs, validation pipeline, multi-tenant isolation, and GDPR-compliant deletion
sidebar:
  order: 7
---
...
Granit.BlobStorage provides sovereign, Direct-to-Cloud file storage where the
application server never handles file bytes -- only metadata and presigned URLs
flow between the client, the server, and S3-compatible object storage.
...
- [Set up localization](/guides/set-up-localization/) to localize blob storage
  error messages (`BlobStorage:NotFound`, `BlobStorage:NotValid`)
- [Set up end-to-end tracing](/guides/end-to-end-tracing/) to trace blob
  operations in Grafana Tempo
- [Blob storage reference](/reference/modules/blob-storage/) for the complete
  API surface
