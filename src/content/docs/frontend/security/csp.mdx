...
description: CSP and Trusted Types compatibility for Granit apps. Recommended directives, the policy-compartment pattern, per-package CSP requirements, and the architecture test that keeps the convention enforced.
sidebar:
...
Shipped by `@granit/react-map/csp`. Covers Leaflet's
