...
description: Keycloak OIDC auth for React with @granit/react-authentication — PKCE, silent SSO, token refresh, back-channel logout, typed context, API keys.
sidebar:
...
`@granit/authentication` defines provider-neutral OIDC types — user claims,
login/logout options, and a base auth context interface. `@granit/react-authentication`
stays provider-agnostic too: it exports the `createAuthContext` and `createMockProvider`
factories so each consuming app can define its own typed auth context.

Keycloak specifics live one package further out: `@granit/authentication-keycloak` holds
the Keycloak types (`KeycloakCoreConfig`, `KeycloakEvent`), and
`@granit/react-authentication-keycloak` provides the `useKeycloakInit` hook that handles
PKCE initialization, silent SSO, automatic token refresh, and back-channel logout.
...
| `@granit/react-authentication` | `createAuthContext`, `createMockProvider` | `@granit/authentication`, `react` |
| `@granit/react-authentication-keycloak` | `useKeycloakInit` | `@granit/authentication-keycloak`, `react`, `keycloak-js` |
...
Standard OIDC user claims from the provider's `/userinfo` endpoint. The type is
provider-neutral — it is not Keycloak-specific.
...
|----------|-------------|---------|
| OIDC types | `OidcUserInfo`, `LoginOptions`, `LogoutOptions` | `@granit/authentication` |
| Auth context | `BaseAuthContextType` | `@granit/authentication` |
| Keycloak types | `KeycloakEvent`, `KeycloakCoreConfig` | `@granit/authentication-keycloak` |
| Keycloak hook | `useKeycloakInit()`, `KeycloakCoreResult` | `@granit/react-authentication-keycloak` |
...
- [.NET counterpart: Authentication](/dotnet/security/authentication/) — the server-side module behind this package
- [Granit.Users module](/dotnet/security/authentication/) — .NET authentication and JWT Bearer middleware
- [API client](/frontend/api/http-client/) — `setTokenGetter` and `setOnUnauthorized` are wired automatically
- [Authorization](/frontend/security/authorization/) — Permission checking after authentication
- [Multi-tenancy](/frontend/infrastructure/multi-tenancy/) — Tenant resolution from JWT claims
