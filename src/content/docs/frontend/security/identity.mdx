...
description: Detect IdP capabilities at runtime with @granit/react-identity — Keycloak and Entra ID feature detection via the useIdentityCapabilities hook.
sidebar:
  label: Identity
  order: 6
---
...
|---------|------|------------|
| `@granit/identity` | `IdentityProviderCapabilitiesResponse` type, `getIdentityCapabilities()` | `axios` |
...
const caps = await getIdentityCapabilities(axiosClient, '/identity/users');
...
```ts
interface IdentityProviderCapabilitiesResponse {
...
### `getIdentityCapabilities(client, basePath)`

Fetches capabilities from `GET {basePath}/capabilities`.

```ts
function getIdentityCapabilities(
...
`IdentityProviderCapabilitiesResponse` and `getIdentityCapabilities` live in `@granit/identity` —
...
|----------|-------------|---------|
| Capabilities type | `IdentityProviderCapabilitiesResponse` | `@granit/identity` |
| API function | `getIdentityCapabilities()` | `@granit/identity` |
...
- [Authentication](/frontend/security/authentication/) — Keycloak/Cognito session that provides the JWT for identity operations
- [Authorization](/frontend/security/authorization/) — Permission management complements identity capabilities
