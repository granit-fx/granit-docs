...
description: Role-based access control in React with @granit/react-authorization — O(1) permission lookups, definition tree, role grant/revoke, TanStack Query cache.
sidebar:
  label: Authorization
  order: 5
---
...
```ts
type PermissionDefinitionResponse = {
  name: string;
...
|----------|-------------|---------|
| Permission types | `PermissionDefinitionResponse`, `PermissionGroupResponse`, `PermissionGrantResponse`, `MyPermissionsResponse` | `@granit/authorization` |
...
- [.NET counterpart: Authorization](/dotnet/security/authorization-roles/) — the server-side module behind this package
- [Granit.Authorization module](/dotnet/security/authentication/) — .NET policy-based authorization
- [Authentication](/frontend/security/authentication/) — Must be authenticated before checking permissions
