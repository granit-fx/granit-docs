...
description: GDPR cookie consent in React with @granit/react-cookies — pluggable CMP adapter, vanilla-cookieconsent integration via @granit/cookies-cookieconsent.
sidebar:
  label: Cookies
  order: 18
---
...
`@granit/cookies` defines an abstract `CookieConsentAdapter` interface and GDPR
category types — mirroring `Granit.Http.Cookies` on the .NET backend.
`@granit/react-cookies` wraps this into a React context. `@granit/cookies-cookieconsent`
provides a concrete adapter backed by vanilla-cookieconsent.

The architecture is pluggable: swap `@granit/cookies-cookieconsent` for any other
CMP (Klaro, Cookiebot, OneTrust, etc.) by implementing `CookieConsentAdapter`.
...
- @granit/cookies/ CMP interface, GDPR categories, backend DTOs (framework-agnostic)
  ...
  - @granit/cookies-cookieconsent vanilla-cookieconsent CMP adapter (static or dynamic config)
</FileTree>

| Package | Role | Depends on |
|---------|------|------------|
| `@granit/cookies` | `CookieConsentAdapter` interface, `CookieCategory`, backend DTOs | — |
| `@granit/react-cookies` | React context provider, `useCookieConsent` hook | `@granit/cookies`, `react` |
| `@granit/cookies-cookieconsent` | `createCookieConsentProvider()` factory | `@granit/cookies`, `vanilla-cookieconsent` |

## Setup

<Tabs>
<TabItem label="React + vanilla-cookieconsent">
...
const cmp = createCookieConsentProvider({
...
All types and the `CookieConsentAdapter` interface live in `@granit/cookies` — pure
...
|----------|-------------|---------|
| Types | `CookieCategory`, `ConsentState`, `CookieConsentConfigResponse`, `CookieDefinitionResponse` | `@granit/cookies` |
| Interface | `CookieConsentAdapter` | `@granit/cookies` |
| React provider | `CookieConsentProvider` (component) | `@granit/react-cookies` |
| React hook | `useCookieConsent()` | `@granit/react-cookies` |
| CMP adapter | `createCookieConsentProvider()` | `@granit/cookies-cookieconsent` |

## See also

- [.NET counterpart: Cookies](/dotnet/compliance/cookies/) — the server-side module behind this package
- [Granit.Http.Cookies module](/dotnet/compliance/cookies/) — .NET cookie consent management (CookieConsent CMP server-side)
