...
description: Capture React errors with @granit/react-error-boundary — headless boundary, global capture, breadcrumb trail, and OpenTelemetry span correlation.
sidebar:
...
- [Logger](/frontend/observability/logger/) — Error boundary logs via `@granit/logger`
- [Tracing](/frontend/observability/tracing/) — Errors are correlated with traces when `TracingProvider` is active
