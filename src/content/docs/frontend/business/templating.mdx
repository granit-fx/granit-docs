...
description: Scriban document templates in TypeScript with @granit/templating — Draft/Published/Archived lifecycle, HTML/PDF/Excel preview, and revision history.
sidebar:
  label: Templating
  order: 13
---
...
`@granit/templating` and its React companion `@granit/react-templating` provide template
...
<FileTree>
- @granit/templating/ Types, API functions (framework-agnostic)
  - @granit/react-templating `TemplatingProvider`, query/mutation hooks
</FileTree>

| Package | Role | Depends on |
|---------|------|------------|
| `@granit/templating` | DTOs, lifecycle enums, API functions | `axios` |
| `@granit/react-templating` | `TemplatingProvider`, query/mutation hooks | `@granit/templating`, `@tanstack/react-query`, `react` |

:::note[Two-tier like every other domain]
Templating follows the standard split: `@granit/templating` holds the DTOs and API
functions with no React dependency, and `@granit/react-templating` adds the provider
and hooks. The API functions can be used standalone without React.
:::
...
```ts
import { listTemplates, saveDraft, publishTemplate, previewTemplate } from '@granit/templating';
...
interface SaveTemplateCategoryRequest {
  readonly name: string;
  readonly description?: string;
  readonly icon?: string;
  readonly sortOrder?: number;
...
|----------|----------|-------------|
| `listTemplates(client, basePath, params?)` | `GET /templates` | Paginated template list with filtering |
...
| Category types | `TemplateCategory`, `TemplateCategoryId`, `SaveTemplateCategoryRequest` | `@granit/templating` |
| API functions | `listTemplates()`, `saveDraft()`, `publishTemplate()`, `previewTemplate()`, `getVariables()` | `@granit/templating` |
...
- [.NET counterpart: Templating](/dotnet/building-blocks/templating/) — the server-side module behind this package
- [Granit.Templating module](/dotnet/building-blocks/templating/) — .NET Scriban template engine with EF store
- [Workflow](/frontend/business/workflow/) — Templates use `WorkflowLifecycleStatus` for publication lifecycle
- [Data Exchange](/frontend/business/data-exchange/) — Export templates can be managed through this system
