...
description: Configure @granit/api-client — Axios factory with Bearer tokens, X-Tenant-Id header, RFC 7807 errors, idempotency keys, and orval mutator support.
sidebar:
  label: API Client
  order: 1
---
...
Creates an [orval-compatible mutator](https://orval.dev/docs/guides/custom-client/) from an
...
| Idempotency | `enableIdempotency()`, `disableIdempotency()`, `IdempotencyClientOptions` | `@granit/idempotency` |
| Test utilities | `createMockClient()`, `axiosResponse()` | `@granit/testing` |
...
- [Authentication](/frontend/security/authentication/) — `setTokenGetter` is wired automatically by `@granit/react-authentication`
- [Multi-tenancy](/frontend/infrastructure/multi-tenancy/) — `setTenantGetter` is wired automatically by `@granit/react-multi-tenancy`
- [OpenAPI Contract Generator](/contributing/openapi-contract-generator/) — where the generated `_generated.ts` types come from, and why `int64` fields are typed `string | number`
