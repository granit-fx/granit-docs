...
description: Build headless data grids with @granit/react-query-engine — typed filters, multi-column sorting, cursor pagination, grouping, and saved views.
sidebar:
  label: QueryEngine
  order: 9
---
...
`useQueryEndpoint` (full query state machine), `usePagedInfiniteQuery`, `useInfiniteScroll`,
`useSmartFilter`, `useQueryMeta`, and `useQueryCatalog`.
...
| `@granit/react-query-engine` | `QueryProvider`, `QueryCatalogProvider`, `useQueryEndpoint`, `usePagedInfiniteQuery`, `useInfiniteScroll`, `useSmartFilter`, `useQueryMeta`, `useQueryCatalog` | `@granit/query-engine`, `@tanstack/react-query`, `react` |
...
```ts
import { getPage, getQueryMeta, serializeQueryRequest } from '@granit/query-engine';
import type { QueryRequest, PagedResult, FilterEntry } from '@granit/query-engine';
```
...
interface ColumnDefinition {
  ...
  readonly label: string;
  readonly type: string;             // CLR type name — "String", "Int32", "Decimal", …
  readonly order: number;
  readonly isSortable: boolean;
  readonly isFilterable: boolean;
  readonly isVisible: boolean;
  readonly format?: string;          // display format hint — "dd/MM/yyyy", "N2", …
  readonly valueKind?: string;       // semantic display type — "Currency", "Url", … (absent ⇒ use type)
  readonly currencyCode?: string;    // ISO 4217, only present when valueKind === "Currency"
}
```

:::tip[Picking a cell renderer: `valueKind` first, `type` second]
`valueKind` is the **highest-priority** signal for choosing a cell renderer — it says
what a value *means* (`"Currency"`, `"Percentage"`, `"Url"`, `"Email"`, `"Bytes"`,
`"RelativeTime"`, …), which a raw CLR `type` cannot. When it is **absent**, degrade to
`type` (the pre-existing behaviour). Read `currencyCode` alongside `valueKind === "Currency"`
to format with the right symbol and locale. Both are metadata-only PascalCase enum names on
the wire; the backend sets them via the [`ColumnBuilder` value-kind helpers](/dotnet/building-blocks/query-engine/#column-display-metadata).

```ts
function pickRenderer(col: ColumnDefinition) {
  switch (col.valueKind) {                 // 1. semantic hint wins
    case 'Currency':   return currencyCell(col.currencyCode ?? 'EUR');
    case 'Percentage': return percentCell;
    case 'Url':        return linkCell;
    case 'Email':      return mailtoCell;
    case 'Bytes':      return byteSizeCell;
  }
  return rendererForClrType(col.type);      // 2. fall back to the CLR type
}
```
...
```ts
function getPage<T>(client, basePath, params: QueryRequest): Promise<PagedResult<T>>;
function getGrouped<T>(client, basePath, params: QueryRequest): Promise<GroupedResult<T>>;
function getQueryMeta(client, basePath): Promise<QueryMetadata>;
...
```ts
function serializeQueryRequest(params: QueryRequest): string;
function parseQueryRequest(search: string): QueryRequest;
...
|----------|-------------|---------|
| Query types | `QueryRequest`, `FilterEntry`, `SortEntry`, `FilterOperator`, `PagedResult<T>`, `GroupedResult<T>` | `@granit/query-engine` |
...
| API functions | `getPage()`, `getGrouped()`, `getQueryMeta()`, saved views CRUD | `@granit/query-engine` |
| Serialization | `serializeQueryRequest()`, `parseQueryRequest()` | `@granit/query-engine` |
...
- [.NET counterpart: QueryEngine](/dotnet/building-blocks/query-engine/) — the server-side module behind this package
- [Granit.QueryEngine module](/dotnet/building-blocks/query-engine/) — .NET whitelist-first query engine with expression trees; source of the `valueKind` / `currencyCode` column metadata this grid renders
- [Notifications](/frontend/infrastructure/notifications/) — Uses `useInfiniteScroll` from this package for the notification inbox
- [Data Exchange](/frontend/business/data-exchange/) — Uses `@granit/utils` for shared formatting
