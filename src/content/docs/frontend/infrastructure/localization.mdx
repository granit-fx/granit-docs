...
description: i18n for React with @granit/localization — i18next integration, backend translations, locale detection cascade, and language switcher hook.
sidebar:
  label: Localization
  order: 17
---
...
import type { LanguageInfo, ApplicationLocalizationResponse } from '@granit/localization';
...
interface ApplicationLocalizationResponse {
  cultureName: string;
...
module (e.g. `{ "Granit": {...}, "MyApp": {...} }`) — this function merges them into
the single `translation` namespace.

```ts
function applyTranslations(instance: i18n, data: ApplicationLocalizationResponse): void;
```
...
|----------|-------------|---------|
| Types | `LanguageInfo`, `ApplicationLocalizationResponse`, `LocalizationConfig` | `@granit/localization` |
...
- [Settings](/frontend/infrastructure/settings/) — `SETTING_NAMES.PREFERRED_CULTURE` stores the user's language preference
- [Reference Data](/frontend/business/reference-data/) — Country labels use the same multilingual pattern
