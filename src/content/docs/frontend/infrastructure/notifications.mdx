...
description: Real-time notification UIs with @granit/react-notifications — SignalR and SSE transports, Web Push, inbox hooks, and user preferences.
sidebar:
  label: Notifications
  order: 8
---
...
| `@granit/react-notifications` | `NotificationsProvider`, inbox/feed/preferences hooks | `@granit/notifications`, `@granit/react-query-engine`, `react` |
...
function App() {
  return (
    <NotificationsProvider config={{ apiClient: api }} transport={transport}>
      <NotificationBell />
...
```ts
import type { UserNotification, NotificationTransport } from '@granit/notifications';
import { listNotifications, markAsRead } from '@granit/notifications';
```
...
Any transport implementing this interface can be plugged into `NotificationsProvider`.

### API functions

| Function | Endpoint | Description |
|----------|----------|-------------|
| `listNotifications(client, basePath, params?)` | `GET /notifications` | Paginated inbox |
...
| `getUnreadCount(client, basePath)` | `GET /notifications/unread-count` | Unread count |
...
|----------|-------------|---------|
| Core types | `UserNotification`, `NotificationSeverity`, `ConnectionState`, `NotificationChannels` | `@granit/notifications` |
| Transport | `NotificationTransport`, `NotificationConfig` | `@granit/notifications` |
| API functions | `listNotifications()`, `markAsRead()`, `markAllAsRead()`, `getUnreadCount()` | `@granit/notifications` |
...
| Provider | `NotificationsProvider`, `useNotificationContext()` | `@granit/react-notifications` |
...
- [QueryEngine](/frontend/data/query-engine/) — Pagination primitives used by the notification inbox
