...
description: Multi-tenant React apps with @granit/react-multi-tenancy — resolver pipeline reads tenant from JWT claims, auto X-Tenant-Id header injection.
sidebar:
  label: Multi-tenancy
  order: 7
---
...
- [.NET counterpart: Multi-Tenancy](/dotnet/infrastructure/multi-tenancy/) — the server-side module behind this package
- [Granit.MultiTenancy module](/dotnet/infrastructure/multi-tenancy/) — .NET tenant isolation and resolver pipeline
- [API client](/frontend/api/http-client/) — `setTenantGetter` is wired automatically by `TenantProvider`
- [Authentication](/frontend/security/authentication/) — Keycloak session that provides the JWT with `tenant_id`
