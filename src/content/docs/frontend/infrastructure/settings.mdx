...
description: Cascaded settings for React — @granit/settings resolves User → Tenant → Global → Config → Default with TanStack Query cache invalidation.
sidebar:
  label: Settings
  order: 15
---
...
```ts
import { getSettings, updateSetting, deleteSetting } from '@granit/settings';
...
|----------|----------|-------------|
| `getSettings(client, basePath, scope)` | `GET /settings/{scope}` | All visible settings for a scope |
| `getSetting(client, basePath, scope, name)` | `GET /settings/{scope}/{name}` | Single setting by name |
...
| API functions | `getSettings()`, `getSetting()`, `updateSetting()`, `deleteSetting()` | `@granit/settings` |
...
- [.NET counterpart: Application Settings](/dotnet/infrastructure/application-settings/) — the server-side module behind this package
