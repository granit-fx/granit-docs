...
description: Install Granit.Mcp.Server to expose MCP tools via Streamable HTTP — module registration, endpoint mapping, authentication, and appsettings.json reference.
sidebar:
  label: Setup
  order: 2
  badge:
...
app.MapGranitMcpServer(options =>
{
    ...
    options.MapDiagnosticsEndpoints = false;
});
```
...
```json
{
  "Mcp": {
    ...
    "Server": {
      ...
      "MapDiagnosticsEndpoints": true,
      "DiagnosticsTagName": "MCP Admin",
      "EnabledModules": []
    }
  }
}
...
| `MapDiagnosticsEndpoints` | `bool` | `true` | Expose tool listing admin API |
| `DiagnosticsTagName` | `string` | `"MCP Admin"` | OpenAPI tag for admin endpoints |
