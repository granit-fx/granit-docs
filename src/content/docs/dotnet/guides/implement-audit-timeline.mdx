...
description: Add an Odoo-style activity feed to any EF Core entity — audit logs, comments, @mentions, threaded replies, attachments, followers, and notifications.
sidebar:
  order: 17
...
Use `ITimelineWriter` to create entries programmatically. Three entry types
are available:
...
Use `ITimelineReader` to retrieve the paginated activity feed for an entity:
...
Each `TimelineEntry` accepts ad-hoc emoji-style reactions (`👍`, `🎉`, `🚀`, …) via the
`Granit.Timeline` reaction API (PR #1875). A reaction carries the author, the reaction
key (`":+1:"`, `":heart:"`, etc.) and the timestamp; one user can hold a single reaction
per entry — calling `ReactAsync` with the same key twice removes it.

```csharp
await timelineWriter.ReactAsync(
    entryId:     entry.Id,
    reactionKey: ":+1:",
    cancellationToken);

await timelineWriter.RemoveReactionAsync(
    entryId:     entry.Id,
    reactionKey: ":+1:",
    cancellationToken);
```

The reader projects an aggregated `IReadOnlyDictionary<string, ReactionTally>` per
entry — `{ ":+1:": { Count: 4, ReactedByMe: true }, … }` — so the UI can render
...
- [Timeline reference](/dotnet/building-blocks/timeline/) -- full API and configuration details
