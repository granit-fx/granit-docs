...
description: Define SaaS feature flags with toggle, numeric, and selection types — Tenant/Plan/Default cascading, quota guards, EF Core, and FusionCache reads.
sidebar:
  order: 9
...
- `Granit.Features` for feature definitions and resolution (framework-pure core)
- `Granit.Http.Features` / `Granit.Features.Wolverine` for endpoint / handler gating (optional)
- `Granit.Features.EntityFrameworkCore` for database-backed overrides (optional)

## Step 1 -- Install packages

The core is framework-pure; gating ships as transport bindings, mirroring
...
```bash
# Core: definitions, resolution, IFeatureChecker, IFeatureLimitGuard
dotnet add package Granit.Features

# Minimal-API endpoint gating: .RequiresFeature()
dotnet add package Granit.Http.Features

# Wolverine handler gating: [RequiresFeature] on the message type
...
The endpoint filter from `Granit.Http.Features` returns HTTP 403 with a
`ProblemDetails` response (`errorCode: "Features:NotEnabled"`) when the feature is
disabled:
...
`Granit.Features.Wolverine` enforces the gate via a pipeline middleware. Decorate
the **message type** — the middleware rejects it before the handler runs:

```csharp
using Granit.Features.Wolverine.Attributes;

[RequiresFeature("App.VideoConsultation")]
public sealed record CreateVideoSessionCommand(Guid Id);
```

Register the middleware once in your Wolverine setup:
