...
description: Wire Granit.Parties.MultiTenancy so each new tenant gets a host-scoped Party that downstream Invoicing, Subscriptions, and Payments modules use.
sidebar:
  order: 31
...
- [Wire a new external provider through Party.ExternalMappings](/dotnet/guides/parties-external-mappings/) — uses the reserved `tenant` mapping seeded here
- [Auto-maintain Party.Roles via integration events](/dotnet/guides/parties-role-handler/) — companion handler pattern
- [Multi-tenancy module](/dotnet/infrastructure/multi-tenancy/) — `OnTenantCreated` lifecycle and provisioning
- [Configure multi-tenancy guide](/dotnet/guides/configure-multi-tenancy/) — step-by-step tenant setup
