...
HandleAsync` method matching the [Granit handler discovery rules](/dotnet/infrastructure/wolverine-messaging/).
...
- Live in a downstream module that depends on `Granit.Parties` — never the reverse.
  The [no-circular-reference architecture test](https://github.com/granit-fx/granit-dotnet/blob/develop/tests/Granit.ArchitectureTests/ProjectDependencyTests.cs)
  guarantees the direction cannot invert: once a module depends on `Granit.Parties`,
  a back-reference would form a cycle and fail the build.
...
- [Parties module](/dotnet/business/parties/) — `Party` aggregate, `PartyRoles` flags, and module reference
- [Seed the default party at tenant provisioning](/dotnet/guides/parties-tenant-seeding/) — sibling guide for tenant lifecycle
- [Wire a new external provider through Party.ExternalMappings](/dotnet/guides/parties-external-mappings/) — sibling guide for billing/accounting providers
- [Wolverine messaging](/dotnet/infrastructure/wolverine-messaging/) — integration event handler conventions
- [Authorization roles](/dotnet/security/authorization-roles/) — distinct from `PartyRoles` (do not confuse RBAC roles with party roles)
