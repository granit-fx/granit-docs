...
description: Build Scriban templates for HTML emails, PDF invoices via PuppeteerSharp, and Excel reports via ClosedXML — Granit's document generation pipeline.
sidebar:
  order: 12
...
Browser-process settings live in `Granit.Browsing`, not in the PDF section. For
Docker deployments, install Chromium system-wide, point the provider at it, and
enable `DisableSandbox` if the container cannot run with `CAP_SYS_ADMIN` or a
...
Pool sizing (`MaxBrowsers`, `MaxPagesPerBrowser`, `AcquireTimeout`,
`MaxRenderDuration`) is configured on the `Browsing` section itself — see
...
[Security section](/dotnet/building-blocks/document-generation/#security) in the module
...
- [Set up localization](/dotnet/guides/set-up-localization/) for culture-specific
  template resolution
- [Templating reference](/dotnet/building-blocks/templating/) for the complete API
  surface
