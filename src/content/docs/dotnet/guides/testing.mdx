...
description: Cut xUnit boilerplate with GranitTestFixture — NSubstitute fakes for IClock, ICurrentTenant, ICurrentUserService, EF Core in-memory, and Bogus generators.
sidebar:
  order: 99
  label: Testing
---
...
|---------|------|------------|
| `Granit.Testing` | `GranitTestFixture`, fakes, `GranitWebApplicationFactory`, Bogus generators | `Granit`, `Granit.Timing`, `Granit.Guids`, `Bogus` |
...
- [Testing guide (contributing)](/contributing/testing-guide/) — repo-wide testing conventions and CI expectations
- [Persistence module](/dotnet/data/persistence/) — `AuditedEntity`, `FullAuditedEntity`, `IMultiTenant`
- [Multi-tenancy module](/dotnet/infrastructure/multi-tenancy/) — `ICurrentTenant` fake setup
- [Wolverine messaging](/dotnet/infrastructure/wolverine-messaging/) — `ICurrentUserService` propagation under test
- [Don't mock the database — use Testcontainers](/blog/dont-mock-the-database-use-testcontainers/) — companion blog post
