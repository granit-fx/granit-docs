...
description: Build a self-contained Granit module — GranitModule base class, [DependsOn] declarations, service registration, lifecycle hooks, and folder structure.
sidebar:
  order: 1
...
Declaring a dependency on `GranitPersistenceEntityFrameworkCoreModule` automatically pulls
in `GranitPersistenceModule`, `GranitTimingModule`, and `GranitGuidsModule`. You do not
need to repeat them. `ICurrentUserService` and the audit primitives ship inside the core
`Granit` package under the `Granit.Users` namespace — no separate `Granit.Users` package
exists.
:::
