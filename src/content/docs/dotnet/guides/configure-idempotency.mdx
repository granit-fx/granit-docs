...
Outside `Development`, startup fails when the resolved `IConditionalCache` is
in-memory: a per-replica idempotency store means a retry landing on another pod
re-executes the handler. Register a distributed cache (Redis) or — for a
single-replica host only — set `Http:Idempotency:AllowInMemoryStore` to `true`.
:::
