...
description: Add Stripe-style idempotency to Minimal API endpoints — Redis state machine, SHA-256 keys, AES-256-GCM, and safe retries for POST/PUT/PATCH.
sidebar:
  order: 19
...
- A Redis instance for production deployments (the in-memory default store is
  Development / single-replica only)

## 1. Install the packages

```bash
dotnet add package Granit.Http.Idempotency
dotnet add package Granit.Http.Idempotency.StackExchangeRedis   # production store
```
...
using Granit.Http.Idempotency.StackExchangeRedis;

[DependsOn(typeof(GranitHttpIdempotencyStackExchangeRedisModule))]
public sealed class MyAppModule : GranitModule { }
```

The Redis module depends on `GranitHttpIdempotencyModule` and replaces the
...
:::caution[Distributed store required outside Development]
Outside `Development`, startup fails when the registered `IIdempotencyStore`
reports `IsDistributed = false`: a per-replica idempotency store means a retry
landing on another pod re-executes the handler. Register the Redis provider or —
for a single-replica host only — set `Http:Idempotency:AllowInMemoryStore` to
`true`.
:::

:::caution[Encryption key required outside Development]
The Redis store encrypts every entry at rest with AES-256-GCM — unconditionally.
Outside `Development`, startup fails closed when no base64 256-bit
`Cache:Encryption:Key` is configured. There is no opt-out.
:::
...
Absent --(SET NX PX)--> InProgress --(SET XX PX)--> Completed
                                                  |
                                                  +--(response > MaxResponseSizeBytes)--> Tombstoned

Tombstoned --(replay)--> 413 Payload Too Large (until TombstoneTtl elapses)
                             |
                             +--(5xx / timeout)--> Absent
```
...
- **Encryption**: With the Redis provider, cached responses are encrypted with
  AES-256-GCM via `ICacheValueEncryptor` — unconditionally, using the
  `Cache:Encryption:Key`. This is mandatory for ISO 27001 compliance when
  response bodies contain health data.
