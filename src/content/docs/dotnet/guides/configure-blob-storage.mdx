...
description: Set up multi-provider file storage (S3, Azure, FileSystem, Database) with presigned URLs, validation, tenant isolation, and GDPR-compliant deletion.
sidebar:
  order: 7
...
Validators run **after** the bytes land in storage (direct-to-cloud upload).
`BlobValidationContext` hands you the descriptor, the actual size from the
HEAD response, and a factory that opens a partial range-GET stream — the full
file is never buffered in the application server.
...
public sealed class AntivirusValidator : IBlobValidator
{
    ...
    {
        await using Stream head = await context.OpenPartialStreamAsync(64 * 1024, cancellationToken);

        // Perform antivirus scan over the range-GET stream
        return BlobValidationResult.Success();
    }
}
```

The pipeline is fail-fast: it stops at the first result whose `IsValid` is
`false`. Register the validator before calling `AddGranitBlobStorageS3()` —
the built-ins are registered as singletons:
