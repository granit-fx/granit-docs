---
title: "Set Up Localization \u2014 i18n for 18 Cultures"
description: Configure JSON-based modular i18n supporting 18 cultures with CLDR plural rules — source-generated IStringLocalizer keys, per-tenant database overrides at runtime, and culture fallback chains.
sidebar:
  order: 10
...
| --- | --- | --- |
| `GET` | `/api/{version}/localization/overrides` | Query-engine list (`page`, `pageSize`, `sort`, `filter[...]`, `search`, `presets[...]`, `groupBy`) |
| `GET` | `/api/{version}/localization/overrides/meta` | Query metadata (columns, filters, sorts, presets) for dynamic UI generation |
...
<Aside type="caution" title="Breaking change — Granit 0.1.0-dev (May 2026)">
The legacy `GET /localization/overrides?resourceName=X&cultureName=Y` lookup is
removed. `MapGranitLocalizationOverrides()` now mounts the query-engine contract
(`GET /` + `GET /meta`); consumers must migrate to filter on `resourceName` /
`cultureName` via `filter[...]` parameters. `PUT` and `DELETE` are unchanged. See
[query-engine](/frontend/data/query-engine/) for the parameter shape.
</Aside>

#### SPA usage

The list and metadata endpoints plug directly into the
[`@granit/react-query-engine`](/frontend/data/query-engine/) hooks. Point
`QueryProvider` at the overrides base path and the hooks fetch `GET /` and
`GET /meta` automatically:
...
const QUERY_CONFIG = { basePath: '/api/v1/localization/overrides' };

export function LocalizationOverridesPage() {
  return (
    <QueryProvider config={QUERY_CONFIG}>
      <LocalizationOverridesContent />
    </QueryProvider>
  );
}

function LocalizationOverridesContent() {
  const meta = useQueryMeta();                          // GET /api/v1/localization/overrides/meta
  const query = useQueryEndpoint<LocalizationOverride>({ // GET /api/v1/localization/overrides
    initialParams: {
      page: 1,
      pageSize: 25,
  ...
  if (meta.isLoading || query.query.isLoading) return null;
  return <pre>{JSON.stringify(query.query.data, null, 2)}</pre>;
}
```

The Granit admin showcase wires this exact pattern in
`granit-showcase-admin-react/src/features/localization/localization-override-list-page.tsx`.
...
Granit supports 18 cultures out of the box: 15 base languages plus 3 regional
variants.
...
Every `src/*/Localization/**/*.json` must exist for all 18 cultures. Regional
