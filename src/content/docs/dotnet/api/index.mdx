...
description: HTTP infrastructure packages — CORS, API versioning, OpenAPI, exception handling, idempotency, rate limiting, bulkhead isolation, and response compression.
sidebar:
  label: Overview
  order: 11
---
...
The rate-limiting core is **framework-pure** — `Granit.Http.RateLimiting` is the
ASP.NET Core binding (endpoint filter + 429 mapping), and `Granit.RateLimiting.Wolverine`
the message binding. See [Rate Limiting](/dotnet/api/rate-limiting/) and
