...
`ApiDocumentation:MajorVersions` is the single source of truth for both the
...
OpenAPI document; the `Scalar:OAuth2` section holds the browser-client bits
...
always; in Production only when `Scalar:EnableInProduction` is `true` — and the
