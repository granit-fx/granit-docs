...
Granit.Http.Idempotency provides Stripe-style HTTP idempotency middleware backed by a
distributed store (Redis via `IConditionalCache`). Ensures that retried POST/PUT/PATCH
requests produce the same response without re-executing side effects. Uses SHA-256
composite keys and AES-256-GCM encrypted entries.
...
## Store: distributed by default

The middleware persists its state machine through `IConditionalCache`
(`Granit.Caching`). The backend is whatever cache provider the host registers —
Redis via `Granit.Caching.StackExchangeRedis` in any real deployment.

**Fail-loud guard:** outside `Development`, the middleware refuses to start with
a non-distributed (in-memory) `IConditionalCache`. An in-memory idempotency
...
| `AllowInMemoryStore` | `false` | Allow a non-distributed `IConditionalCache` outside Development (per-replica store — see above) |
...
| Store | `IIdempotencyStore`, `IdempotencyEntry`, `IdempotencyState` | `Granit.Http.Idempotency` |
