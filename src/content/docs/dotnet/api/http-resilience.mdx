...
description: Outbound HTTP resilience on Polly v8 — retry, circuit breaker, timeout, and hedging via a single AddGranitHttpClient() extension with appsettings configuration.
sidebar:
...
| `Granit.Notifications.SendGrid` | `SendGrid` | Email |
| `Granit.Notifications.Scaleway` | `Scaleway` | Email |
| `Granit.Notifications.GoogleFcm` | `GoogleFcmPush` | Mobile push |
| `Granit.Identity.Keycloak` | `Identity:Federated:Keycloak` | Admin API |
...
```json
"Http:Resilience": {
  "Identity:Federated:Keycloak": { "Retry": { "MaxRetryAttempts": 1 } },
