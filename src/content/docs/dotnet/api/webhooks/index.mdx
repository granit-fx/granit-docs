...
description: Outbound webhooks with HMAC-SHA256 signed deliveries, Wolverine durable retry, tenant-scoped subscriptions, and ISO 27001-compliant immutable audit trails.
sidebar:
  label: Overview
  order: 1
---
...
Target URLs are validated through [`Granit.Http.Security`](/dotnet/api/url-safety/)
— the shared SSRF primitive extracted from `Granit.Webhooks` (which previously
shipped a private `WebhookSsrfGuard`) and `Granit.Browsing`. Two layers, same
primitive:

1. **At registration** — a FluentValidation `MustAsync` rule calls
   `IUrlSafetyValidator.ValidateAsync` and blocks `http://` by default, private
   IPs (RFC 1918), loopback, link-local, carrier-grade NAT (`100.64.0.0/10`),
   cloud metadata endpoints (`169.254.169.254`), and reserved TLDs (`.local`,
   `.internal`, `.localhost`, `.onion`, `.test`, `.example`, `.invalid`).
2. **At delivery** — a `SocketsHttpHandler.ConnectCallback` consults
   `PrivateNetworkClassifier.IsBlocked(IPAddress)` against the resolved address
   at the moment the socket is opened, defeating DNS rebinding where a hostname
   resolves to a public IP during validation but to an internal IP during
   delivery.

Tune the policy through `UrlSafetyOptions` (`AllowedSchemes`,
`AllowedHostPatterns`, `DeniedHostPatterns`, `MaxUrlLength`) — see the
[URL safety reference](/dotnet/api/url-safety/) for the full surface.
...
| `webhooks.signing_key_rotation_due` | `WebhookSigningKeyRotationDueEto` (emitted daily by the [rotation scanner](/dotnet/api/webhooks/signing-keys/)) | Email, InApp | Warning |

The `signing_key_rotation_due` notification is the operator-facing arm of the
[dual-key rotation flow](/dotnet/api/webhooks/signing-keys/) — it gives administrators a
...
- [Signing keys (dual-key rotation)](/dotnet/api/webhooks/signing-keys/) — `WebhookSigningKey` aggregate, overlap rotation, verification fallback
- [Webhook endpoints](/dotnet/api/webhooks/endpoints/) — full Minimal API surface (subscriptions, deliveries, stats)
- [API & Web module](/dotnet/api/) — Exception handling, versioning, idempotency
- [Persistence module](/dotnet/api/webhooks/persistence/) — EF Core interceptors, query filters
- [Wolverine module](/dotnet/api/webhooks/wolverine/) — Messaging, transactional outbox
