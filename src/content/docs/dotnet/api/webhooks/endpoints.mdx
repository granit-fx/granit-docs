---
title: "Webhooks Endpoints — the admin surface you'd otherwise build by hand"
description: Minimal API for webhook subscription administration — CRUD, lifecycle (active/suspended/deactivated), dual-key signing rotation with overlap, Stripe-style test ping, 24h delivery stats, queryable subscription + delivery lists. SSRF-hardened, permission-scoped, OpenAPI-typed.
sidebar:
  label: Endpoints
  order: 2
  badge:
...
- **SSRF protection (registration)** — blocks private IP ranges (`10.0.0.0/8`,
  ...
  [`Granit.Http.Security`](/dotnet/http/security/url-safety/).
...
- [Granit.Http.Security](/dotnet/http/security/url-safety/) — the SSRF validator behind URL checks
