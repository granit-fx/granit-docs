...
description: Admin API for the full blob lifecycle — initiate direct-to-cloud uploads with presigned URLs, confirm with a validation pipeline, generate download URLs, crypto-shred on delete, and clean up orphans. Per-operation rate limits, queryable descriptors, permission-scoped.
sidebar:
