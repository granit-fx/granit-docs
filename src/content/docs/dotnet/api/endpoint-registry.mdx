...
**Prefix:** `lookups` *(`DataLookupEndpointsOptions.RoutePrefix`)*
...
**Prefix:** host-supplied — `MapGranitODataEndpoints(prefix, …)` for the
per-tenant feed and `MapGranitODataHostEndpoints(prefix, …)` for the
cross-tenant host feed. The examples below use `odata` and `odata/host`.
