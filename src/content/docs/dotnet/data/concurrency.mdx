...
description: Prevent silent data loss with EF Core optimistic concurrency — IConcurrencyAware, ConcurrencyStampInterceptor, automatic HTTP 409 responses.
sidebar:
...
:::caution[Don't hand-roll a version column]
Do not add a manual `RowVersion`/`Version` column with `.IsConcurrencyToken()`
and hand-incremented values. It is forgettable (a missed bump silently drops the
guard), non-portable (`uint` has no native PostgreSQL/SQL Server type), and leaks
write-count. A human-facing "version N" is a *different* concern — use
`IVersioned`. See [ADR-061](/dotnet/architecture/adr/061-optimistic-concurrency-strategy/).
:::
...
## Escape hatch: provider-native tokens

The opaque stamp is the default because it is portable (PostgreSQL, SQL Server,
**and** SQLite) and auto-managed through one code path. The interceptor cannot
guard against writers that **bypass EF Core** (raw SQL, replication, another
service writing the same table). For that — and only that — an entity may opt
into a provider-native token (PostgreSQL `xmin`, SQL Server `rowversion`)
configured explicitly and dispatched via `GranitDbProviders`, with a documented
SQLite/InMemory fallback. This mirrors the advisory-lock pattern in
`EntityMergeConcurrencyLock`. Do not use generated triggers — the framework ships
no schema DDL.
...
- [ADR-061: Optimistic concurrency strategy](/dotnet/architecture/adr/061-optimistic-concurrency-strategy/) — the decision record behind this primitive
