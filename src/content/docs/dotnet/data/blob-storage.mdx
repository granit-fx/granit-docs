...
description: Multi-provider blob storage (S3, Azure, GCS, FileSystem, DbStore) with presigned URLs, proxy endpoints, validation pipeline, tenant isolation.
sidebar:
  label: Blob Storage
  order: 16
---
...
| Extensions | `AddGranitBlobStorageS3()`, `AddGranitBlobStorageAzureBlob()`, `AddGranitBlobStorageGoogleCloud()`, `AddGranitBlobStorageFileSystem()`, `AddGranitBlobStorageDatabase()`, `AddGranitBlobStorageProxy()`, `AddGranitBlobStorageEntityFrameworkCore()`, `MapGranitBlobStorageProxy()` | -- |
