---
title: "Database Migrations — Expand & Contract for Zero-Downtime Deployments"
description: Three EF Core migration approaches for Granit — CLI mode for production, standard EF Core for dev, and Expand & Contract for zero-downtime changes.
sidebar:
  label: Migrations
  order: 5
---
...
|---|---|---|---|---|
| **Package** | `Granit.Persistence.EntityFrameworkCore.Hosting` | EF Core built-in | EF Core built-in | `Granit.Persistence.EntityFrameworkCore.Migrations` |
...
`Granit.Persistence.EntityFrameworkCore.Hosting` adds a first-class `--migrate` CLI mode to any Granit application.
...
1. **Add the package**
   ...
   ```bash
   dotnet add package Granit.Persistence.EntityFrameworkCore.Hosting
   ```
...
**For PostgreSQL**, `Granit.Persistence.EntityFrameworkCore.Postgres` provides `NpgsqlAdvisoryMigrationLock`
...
`Granit.Persistence.EntityFrameworkCore.Migrations` implements the **Expand & Contract** pattern: split a
...
`Granit.Persistence.EntityFrameworkCore.Migrations` ships both dispatch flavours from a
single package. The default is in-memory `Channel<T>`; activate Wolverine by adding a
reference to `Granit.Wolverine.*` — the migration package detects the registered
`ICommandSender` at startup and routes batches through the durable outbox instead.

| | Default (Channel) | With Wolverine |
|---|---|---|
...
Wire up Wolverine when batch processing must survive application restarts or run across
multiple nodes.
