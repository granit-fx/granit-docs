---
title: "Vault providers — HashiCorp, Azure, AWS, GCP"
description: Auth, config, and health checks for Granit.Vault's four providers — HashiCorp Vault, Azure Key Vault, AWS KMS/Secrets Manager, and GCP KMS/Secret Manager.
sidebar:
  order: 4
  label: Providers
---
...
| Native MAC ([`ITransitMacService`](/dotnet/data/vault/transit-mac/)) | Transit HMAC | Managed HSM `HS256` (premium) | KMS `HMAC_256` | Cloud KMS `HMAC_SHA256` |
...
The HashiCorp provider implements [`ITransitMacService`](/dotnet/data/vault/transit-mac/)
...
Azure splits by tier. **Managed HSM** (premium) exposes a native `HS256`
...
Azure Key Vault **Standard** tier has no HMAC primitive — register the portable
[`SecretBackedMacService`](/dotnet/data/vault/transit-mac/#secretbackedmacservice--portable-fallback)
...
AWS KMS implements [`ITransitMacService`](/dotnet/data/vault/transit-mac/) via
...
Google Cloud implements [`ITransitMacService`](/dotnet/data/vault/transit-mac/)
...
Provision the CryptoKey via Terraform with `purpose = "MAC"`,
...
<CardGrid>
  ...
  <LinkCard title="Vault-backed MAC" href="/dotnet/data/vault/transit-mac/" description="ITransitMacService integrity tags and the provider matrix" />
