...
description: Granit.Vault wraps HashiCorp Vault, Azure Key Vault, AWS Secrets Manager, and GCP Secret Manager behind three provider-agnostic .NET abstractions.
sidebar:
  order: 6
  label: Vault
---
...
<CardGrid>
  ...
  <LinkCard
    title="I need a tamper-proof integrity tag (HMAC)"
    href="/dotnet/data/vault/transit-mac/"
    description="ITransitMacService — sign and verify bytes through the vault's native HMAC primitive. Powers signed webhooks, download tokens, and personal-data export integrity." />
  <LinkCard
...
| `Granit.Encryption.EntityFrameworkCore` | `IReEncryptionService`, `DefaultReEncryptionService`, `AddGranitEncryptionReEncryption<TContext>()` | `Granit.Encryption`, `Granit.Persistence.EntityFrameworkCore` |
...
- [Secrets](/dotnet/data/vault/secrets/) — `ISecretStore`, references like `vault:secret/data/foo#key`, caching
- [Credentials](/dotnet/data/vault/credentials/) — dynamic database credentials, automatic rotation
- [Encryption](/dotnet/data/vault/encryption/) — transit encryption keys, wrapping, key rotation
- [Transit MAC](/dotnet/data/vault/transit-mac/) — `ITransitMacService` integrity tags, provider matrix, `SecretBackedMacService` fallback
- [Providers](/dotnet/data/vault/providers/) — HashiCorp Vault, Azure Key Vault, AWS Secrets Manager, GCP Secret Manager

## See also

- [Field-level encryption](/dotnet/data/encryption/) — `[Encrypted]` attribute, EF Core convention, re-encryption job
- [Privacy export signing](/dotnet/compliance/privacy/export-signing/) — `Granit.Privacy.Vault`, the production consumer of `ITransitMacService`
...
- [Encrypt sensitive data guide](/dotnet/guides/encrypt-sensitive-data/) — step-by-step recipe
- [Blog: Secrets management with HashiCorp Vault in .NET 10](/blog/secrets-management-with-hashicorp-vault-in-dotnet-10/) — end-to-end walkthrough of the Vault integration powering this module
