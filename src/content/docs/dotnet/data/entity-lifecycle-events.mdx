...
description: Auto-emit domain events on entity create, update, and delete — implement IEmitEntityLifecycleEvents and the framework dispatches local and distributed ETOs.
sidebar:
...
after `SaveChanges`. With `GranitEventsModule`, they route to registered
...
[DependsOn(
    ...
    typeof(GranitEventsWolverineModule))]  // Wolverine replaces NullIntegrationEventDispatcher
public sealed class AppModule : GranitModule { }
```

`GranitEventsWolverineModule` registers `WolverineIntegrationEventDispatcher`, which
...
Without `GranitEventsWolverineModule`, `IIntegrationEventDispatcher` resolves to
