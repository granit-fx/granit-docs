...
`ISoftDeletable`, and [`IHasMergeTombstone`](/dotnet/data/mergeable/) — a
...
| [`IHasMergeTombstone`](/dotnet/data/mergeable/#ihasmergetombstone--state-only) | Merge lineage | `MergedIntoId` / `MergedAt` columns + `MergeTombstone` query filter |
...
- [Mergeable](/dotnet/data/mergeable/) — `IHasMergeTombstone`, sibling marker
