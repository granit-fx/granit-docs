---
title: Entity View
description: Let users save their own grid layouts — column visibility, sort, filters — with Personal/Shared/Tenant scoping, audited and discoverable.
sidebar:
  label: Entity View
  order: 6
  badge:
...
host's locale via `Validation:*` keys (auto-mapped from FluentValidation's
...
- [Query Engine](/dotnet/building-blocks/query-engine/) — the `QueryDefinition` an `EntityView` overlays
- [Timeline](/dotnet/building-blocks/timeline/) — activity feed for entities
