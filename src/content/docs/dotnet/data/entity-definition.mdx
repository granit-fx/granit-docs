...
description: Define an entity's whole UI surface once — forms, lists, kanban, detail views — rendered automatically. No per-entity React components.
sidebar:
...
A form field can carry a semantic **value-kind** — the same display-type vocabulary a
query column uses ([`ValueKind`](/dotnet/building-blocks/query-engine/#column-display-metadata),
...
hint is set). See [ADR-041](/dotnet/architecture/adr/041-component-catalog/) for the
...
- [Query Engine](/dotnet/building-blocks/query-engine/) — the read-side twin: `ColumnBuilder` value-kind helpers steer grid cell renderers the same way `FieldBuilder.ValueKind()` steers form inputs
...
- [ADR-053](/dotnet/architecture/adr/053-entities-customization-layer-1/) — layer 1 customization (reorder, regroup, hide)
