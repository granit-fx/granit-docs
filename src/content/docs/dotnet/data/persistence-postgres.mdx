...
description: Granit.Persistence.EntityFrameworkCore.Postgres — opinionated Npgsql defaults, pg_try_advisory_lock migration lock, schema-per-tenant, health check.
sidebar:
...
`Granit.Persistence.EntityFrameworkCore.Postgres` is the PostgreSQL provider for Granit's persistence layer.
...
- Granit.Persistence.EntityFrameworkCore.Postgres/
  ...
  - GranitPersistenceEntityFrameworkCorePostgresModule.cs module entrypoint
...
`GranitPersistenceEntityFrameworkCorePostgresModule` depends on `GranitPersistenceEntityFrameworkCoreHostingModule` — you
do not need to declare both.

:::caution
Register this module **before** `GranitPersistenceEntityFrameworkCoreHostingModule` in the dependency
...
Called automatically by `GranitPersistenceEntityFrameworkCorePostgresModule`. Registers three services
...
|--------|------|-------------|
| `GranitPersistenceEntityFrameworkCorePostgresModule` | Module | Entry point — `[DependsOn(GranitPersistenceEntityFrameworkCoreHostingModule)]` |
