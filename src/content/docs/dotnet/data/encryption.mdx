...
:::caution[`[Encrypted]` requires an `IsolatedDbContext`]
An architecture test (PR #2062) pins `[Encrypted]` properties to entities mapped on an
`IsolatedDbContext` — i.e. a tenant-scoped DbContext registered via
`AddGranitIsolatedDbContext<T>()`. Encrypted columns on a host-shared context would
mix per-tenant keys with cross-tenant data; the rule fails the build before that
