...
description: Transparent EF Core field-level AES via [Encrypted] attribute, ApplyEncryptionConventions, and a background key-rotation job. GDPR Art. 32 compliant.
sidebar:
  order: 5
  label: Encryption
---
...
manual calls. When compliance requires key rotation, `IReEncryptionService` re-encrypts
...
- Granit.Encryption/ IStringEncryptionService, AES-256-CBC default
  - Granit.Encryption.EntityFrameworkCore/ [Encrypted] attribute, EncryptedStringConverter, ApplyEncryptionConventions, IReEncryptionService

</FileTree>

| Package | Role | Depends on |
|---------|------|------------|
| `Granit.Encryption` | `IStringEncryptionService` abstraction, in-memory AES-256-CBC default provider | `Granit` |
| `Granit.Encryption.EntityFrameworkCore` | `[Encrypted]` attribute, `EncryptedStringConverter`, `ApplyEncryptionConventions` extension, `IReEncryptionService` + `AddGranitEncryptionReEncryption<TContext>()` | `Granit.Encryption`, `Granit.Persistence.EntityFrameworkCore` |

:::caution[`[SensitiveData]` and `[Encrypted]` are bridged by an architecture test]
The two markers are deliberately separate: `[SensitiveData]` is a **classification**
consumed by the audit, log, AI and export paths to mask or omit a value at runtime,
while `[Encrypted]` is **storage protection** driving the EF Core value converter and
the Wolverine JSON resolver.

`SensitiveDataEncryptionConventionTests` pins the conjunction: every `string` property
carrying `[SensitiveData(Level >= Confidential)]` on a **persisted** type — entity,
saga, or integration event — must also carry `[Encrypted]`. A confidential value that
lands plain-text in the database or the outbox is a GDPR Art. 32 / ISO 27001 A.8.2
violation, so the build fails before it ships. Transient DTOs (`*Request`,
`*Response`) are exempt: they flow over HTTP and never reach a persistence boundary.
:::
...
1. **Add the NuGet package**
   ...
   The re-encryption service ships in the same package — no extra reference needed.
...
Inject `IReEncryptionService` and run it — for example from an admin endpoint, a
hosted service, or a CLI command:

```csharp
public sealed class KeyRotationJob(IReEncryptionService reEncryption)
{
...
`DefaultReEncryptionService` loads entities in batches, marks every `[Encrypted]` property
...
sequenceDiagram
    participant SRE
    participant Vault
    participant Job as IReEncryptionService
    participant DB
...
Lives in `Granit.Encryption.EntityFrameworkCore`. The service is invoked on demand —
it is not itself a recurring Granit background job; wire it into your own scheduled job,
...
Registers `IReEncryptionService` → `DefaultReEncryptionService<TContext>` (scoped).
...
- **Key rotation** is a compliance requirement — `IReEncryptionService` handles this without downtime
...
`IReEncryptionService.ReEncryptAsync` loads entities in batches. If your table has millions
...
- [Vault-backed field encryption](/dotnet/data/vault/encryption/) — key rotation and server-side rewrap via Vault Transit
