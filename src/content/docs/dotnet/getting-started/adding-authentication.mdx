...
description: Add JWT Bearer authentication to your Granit API in 15 minutes — Keycloak or Entra ID claims, audit field population, and RequireAuthorization().
sidebar:
...
Add `.RequireAuthorization()` to the route group you created in the persistence
tutorial. In `Endpoints/TaskEndpoints.cs`:
...
Every endpoint registered on the group — list, get, create, update, delete —
inherits the authorization requirement. No per-endpoint attributes needed.
