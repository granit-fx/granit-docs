...
`MapGranitPrivacyExportDownload()` extension still exists (since the layer-purity
split it ships in the dedicated `Granit.Privacy.BlobStorage.Endpoints` package)
but registers a conflicting `GET /exports/{id}/download` — calling both is a
duplicate-route error. Use `MapGranitPrivacy()` alone.
</Aside>
...
```csharp
[DependsOn(
    typeof(GranitPrivacyWolverineModule),               // export/deletion sagas
...
integration which purges the row and cache entry on `IdentityUserDeletedEto`.
</Aside>
