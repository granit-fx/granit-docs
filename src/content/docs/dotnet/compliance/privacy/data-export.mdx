...
The assembler produces **one or more ZIP64 shards** plus a **separate signed
...
<Aside type="caution" title="Production needs a vault-backed signer">
The default signer generates a per-process key and is **development-only** —
Staging and Production refuse to boot with it. Install `Granit.Privacy.Vault` to
sign fragments and manifests with a key that lives in your vault and survives
restarts and scale-out. See [Export Signing](/dotnet/compliance/privacy/export-signing/).
</Aside>
