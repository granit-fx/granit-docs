...
box. See [ADR-021](/dotnet/architecture/adr/021-privacy-data-export-defaults/) for
...
`Granit.Presence` ships **no** DSAR contributor. The module stores only the
...
- [Blob storage](/dotnet/data/blob-storage/) — where the export archive lands
- [Notifications module](/dotnet/infrastructure/notifications/) — channel that delivers the download link
