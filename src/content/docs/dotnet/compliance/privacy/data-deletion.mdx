...
<Aside type="note" title="DeletionExecutedEto is not an ack barrier">
The deletion saga publishes `PersonalDataDeletionRequestedEto` and
`DeletionExecutedEto` **together** — `DeletionExecutedEto` marks "the erasure was
*requested* of every provider", not "every provider has *finished*". The
`PersonalDataDeletedEto` fragments each provider emits are an audit ledger (per-provider
action + count), not a completion gate; nothing blocks `DeletionExecutedEto` on them.
Providers that need durable, retried erasure should rely on Wolverine's delivery
guarantees on `PersonalDataDeletionRequestedEto`, not on saga aggregation.
</Aside>
