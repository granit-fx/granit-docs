...
description: Granit.Privacy.Regulations — 14 built-in profiles (GDPR, LGPD, CCPA, PIPL, DPDPA, more) with per-tenant resolution via IPrivacyRegulationResolver.
sidebar:
  order: 2
  label: Regulations
---
...
`TenantBasedRegulationResolver` resolves the applicable `PrivacyRegulationProfile` for the
current request using a four-step chain, evaluated in priority order:

<Steps>

1. **`ICurrentTenant.Jurisdiction`** — `TenantResolutionMiddleware` populates this from
   `Tenant.Jurisdiction` (the database column) as part of its existing `FindByIdAsync` call.
   There is no extra database round trip.

2. **`Privacy:Regulations:TenantRegulations[<tenantId>]`** — a per-tenant config entry keyed
   by the tenant's GUID. Use this as an operator override when the stored jurisdiction is absent
   or incorrect without redeploying.

3. **`Privacy:Regulations:DefaultRegulation`** — the global fallback applied when neither of
   the above produces a match.

4. **`InvalidOperationException`** — thrown if all three sources fail to resolve a regulation.
   This prevents requests from proceeding without a known compliance profile.
...
<Aside type="tip">
**Jurisdiction is free when `Granit.MultiTenancy` is included.**
`TenantResolutionMiddleware` reuses its existing tenant lookup to populate
`ICurrentTenant.Jurisdiction` — no additional database call is made.
Enable it by setting `ValidateTenantExistence: true` in your multi-tenancy options
(the default). When `ValidateTenantExistence` is `false`, the middleware skips the
lookup, `Jurisdiction` is `null`, and the resolver falls through to the config-based
steps above.
</Aside>

<Aside type="caution">
`TenantRegulations` keys are **tenant GUIDs** (`ICurrentTenant.Id` is a `Guid?`),
not friendly slugs. The resolver looks up `tenantId.ToString()`, so a non-GUID key
never matches and the tenant silently falls back to the next step in the chain.
</Aside>
...
- [Data Export](/dotnet/compliance/privacy/data-export/) — deadline-driven export workflow
- [Data Deletion](/dotnet/compliance/privacy/data-deletion/) — deletion deadlines per regulation
- [Opt-Out (CCPA)](/dotnet/compliance/privacy/opt-out/) — CCPA-specific sale/share opt-out
- [Processing Purposes](/dotnet/compliance/privacy/processing-purposes/) — legal basis tied to active profile
- [Regulation Bridge](/dotnet/compliance/cookies/regulation-bridge/) — surfaces profile choices into cookie consent
- [Multi-tenancy module](/dotnet/infrastructure/multi-tenancy/) — per-tenant `IPrivacyRegulationResolver`
