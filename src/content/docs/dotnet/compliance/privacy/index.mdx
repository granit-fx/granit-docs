...
- Granit.Privacy Data export saga, deletion with cooling-off, legal agreements
  ...
  - Granit.Privacy.BlobStorage Scatter-gather export pipeline — `PrivacyFragmentUploader` + `ExportArchiveAssemblyHandler` (ZIP + manifest)
...
| `Granit.Privacy.BlobStorage` | Scatter-gather export pipeline: `PrivacyFragmentUploader` (provider-side presigned upload) + `ExportArchiveAssemblyHandler` (ZIP + `manifest.json`, `CountingStream`-guarded against `ExportMaxSizeMb`) | `Granit.Privacy`, `Granit.BlobStorage` |
