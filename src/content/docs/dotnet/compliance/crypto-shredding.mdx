...
description: Per-entity key isolation and cryptographic erasure — destroy encryption keys to permanently wipe data while keeping rows for audit and legal holds.
sidebar:
  order: 3
...
2. **Register the modules**
   ...
   :::danger[No in-memory key store in production]
   The DI default, `InMemoryEntityEncryptionKeyStore`, is **Development-only**. Its keys are
   volatile — a restart loses them, which corrupts existing ciphertext and defeats durable
   crypto-shredding. Outside `Development` it now **throws at resolution** with a message
   pointing at a Vault-backed store (mirroring the AES provider guard). Production hosts must
   register a durable `IEntityEncryptionKeyStore` — load `GranitVaultHashiCorpModule` (as
   above) or supply your own KMS-backed implementation.
   :::
...
encryption is simpler, faster, and supports key rotation via `IReEncryptionService`.
...
```csharp
// Inside a DbContext deriving from GranitDbContext.
...
calls all registered `ICryptoShreddingAuditRecorder` implementations **twice** per key.

### Two-phase erasure (GDPR Art. 5(2))

Key destruction is irreversible, so the audit trail must survive it. `DefaultCryptoShredder`
records the intent **before** destroying the key and the outcome **after**:

1. `RecordAsync(..., CryptoShreddingPhase.Requested, ...)` — written durably *before* the
   Vault key is deleted. From this point a trail exists even if the process crashes between
   phases.
2. The per-entity key is destroyed (`IEntityEncryptionKeyStore.DeleteKeyAsync`).
3. `RecordAsync(..., CryptoShreddingPhase.Confirmed, ...)` — written *after* destruction.

The audit fan-out no longer aborts on the first failing recorder: failures across all
registered recorders are aggregated so every recorder is still notified and the caller sees
every error. `ShredBatchAsync` follows the same ordering per entity.
    ...
    ITimelineWriter writer) : ICryptoShreddingAuditRecorder
{
    ...
    {
        string headline = phase == CryptoShreddingPhase.Requested
            ? "**Encryption key destruction requested** (crypto-shredding)"
...
`DefaultCryptoShredder` invokes every registered recorder twice per key — once with
`CryptoShreddingPhase.Requested` before destruction, once with `Confirmed` after —
aggregating any recorder failures rather than aborting on the first.

### `CryptoShreddingPhase`

```csharp
public enum CryptoShreddingPhase
{
    Requested,  // intent recorded durably BEFORE the key is destroyed
    Confirmed   // recorded AFTER destruction completes
}
```

Identifies which phase of the two-phase erasure a given audit record captures. The
`Requested` record guarantees a durable trail exists even if the process crashes before
`Confirmed` is written (GDPR Art. 5(2)).
...
## `[SensitiveData]` → `[Encrypted]` arch test

A common slip: someone marks a column `[SensitiveData(Sensitivity.Confidential)]`
for log/audit masking but forgets `[Encrypted]` for at-rest protection. The two
attributes serve different layers (telemetry / persistence) and both matter.

Since stage 3/3 of the encryption convention rollout
([#1909](https://github.com/granit-fx/granit-dotnet/pull/1909)), an architecture
test on the CI enforces it: any property carrying
`[SensitiveData(Sensitivity >= Confidential)]` **must** also carry `[Encrypted]`
(with or without `KeyIsolation`). The build fails otherwise — the convention is no
longer aspirational.

## Wolverine field-level encryption — messages in flight

Persistence isn't the only place PII rests. A Wolverine message sitting in the
outbox table, or replayed from the durable inbox, carries the same bytes as the
DbContext write that produced it. `Granit.Wolverine.Encryption`
([#1908](https://github.com/granit-fx/granit-dotnet/pull/1908)) closes that gap:
every `[Encrypted]`-marked property on a Wolverine message envelope is encrypted
before serialization and decrypted on dispatch — using the same key ring and the
same crypto-shred semantics as the DbContext path.

Net result: shredding the key removes the PII from the database, the outbox, the
inbox, and any replay log in one go.
...
- [Blog: Crypto-shredding: GDPR erasure without deleting rows](/blog/crypto-shredding-gdpr-erasure-without-deleting-rows/) — the regulatory case for crypto-shredding and how Granit implements it
