---
title: "Cookies — Secure Cookie Management"
description: Strict cookie registry with consent category enforcement, GPC signal support, and CookieConsent CMP integration. Every cookie must be declared at startup.
sidebar:
  order: 4
  label: Overview
---

## Why a cookie module?

Granit.Http.Cookies enforces a strict cookie registry: every cookie must be declared at
startup with a consent category. Writing an undeclared cookie throws
`UnregisteredCookieException`. Consent is resolved per-category via a pluggable
`IConsentResolver`.

## Package structure

| Package | Role | Depends on |
| ------- | ---- | ---------- |
| `Granit.Http.Cookies` | Cookie registry, consent enforcement, GPC signal, consent ledger contracts | `Granit` |
| `Granit.Http.Cookies.CookieConsent` | CookieConsent CMP consent resolver | `Granit.Http.Cookies` |
| `Granit.Http.Cookies.Endpoints` | Cookie configuration + consent recording endpoints | `Granit.Http.Cookies` |
| `Granit.Http.Cookies.EntityFrameworkCore` | Consent ledger persistence (isolated DbContext) | `Granit.Http.Cookies`, `Granit.Persistence.EntityFrameworkCore` |
| `Granit.Privacy.Regulations.Cookies` | Regulation-aware consent model bridge | `Granit.Http.Cookies`, `Granit.Privacy.Regulations` |

## Setup

```csharp
[DependsOn(typeof(GranitHttpCookiesModule))]
public class AppModule : GranitModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.AddGranitCookies(cookies =>
        {
            cookies.RegisterCookie(new CookieDefinition(
                "session_id", CookieCategory.StrictlyNecessary,
                1, true, "Session identification"));

            cookies.RegisterCookie(new CookieDefinition(
                "analytics_consent", CookieCategory.Analytics,
                365, false, "Analytics tracking preference"));

            cookies.UseCookieConsent();
        });
    }
}
```

## Module auto-registration

Granit modules that introduce cookies auto-declare them via `ICookieDefinitionContributor`.
The cookie registry collects all contributors at first resolution — no manual declaration
needed for framework cookies.

| Module | Cookies auto-registered | Contributor |
| ------ | ----------------------- | ----------- |
| `Granit.Http.Cookies` | `__Host-xsrf` / `.xsrf` (dev) | `AntiforgeryCookieDefinitionContributor` |
| `Granit.OpenIddict.EntityFrameworkCore` | `__Host-id` / `.id` (dev), `__Host-id-2fa` / `.id-2fa`, `__Host-id-ext` / `.id-ext` | `IdentityCookieDefinitionContributor` |
| `Granit.Http.Cookies.CookieConsent` | `cc_cookie` (configurable) | `CookieConsentCookieDefinitionContributor` |
| `Granit.Bff.Endpoints` | `__Host-bff-{name}` / `.bff-{name}` (dev) | Runtime registration in `MapGranitBff()` |
| `Granit.Privacy.Endpoints` | `_optout_id` | Runtime registration in `MapGranitPrivacy()` |

### Implementing a contributor

```csharp
internal sealed class MyCookieContributor : ICookieDefinitionContributor
{
    public IEnumerable<CookieDefinition> GetCookieDefinitions()
    {
        yield return new CookieDefinition(
            "my_cookie", CookieCategory.StrictlyNecessary,
            1, true, "Purpose description")
        { IsEssential = true };
    }
}
```

Register it in your module's `ConfigureServices`:

```csharp
context.Services.AddSingleton<ICookieDefinitionContributor, MyCookieContributor>();
```

### Application-level cookies

The antiforgery cookie (`__Host-xsrf`) is auto-registered by `GranitHttpCookiesModule`.
The session cookie is opt-in because `AddSession()` is not always used. Call
`RegisterSessionCookie()` in hosts that enable session middleware:

```csharp
context.Services.AddGranitCookies(cookies =>
{
    cookies.RegisterSessionCookie();
});
```

## Cookie categories

| Category | Consent required | Description |
| -------- | ---------------- | ----------- |
| `StrictlyNecessary` | No | Session, CSRF, authentication, opt-out tracking |
| `Preferences` | Yes | Language, theme, layout |
| `Analytics` | Yes | Usage tracking (first-party) |
| `Marketing` | Yes | Advertising, retargeting |
| `SaleOrSharing` | Yes | CCPA "Do Not Sell or Share" — third-party data sharing |

## Managed cookie operations

Always use `IGranitCookieManager` instead of `IResponseCookies`:

```csharp
public class SessionService(IGranitCookieManager cookieManager)
{
    public async Task SetSessionCookieAsync(
        HttpContext httpContext, string sessionId)
    {
        // Checks registry, verifies consent, applies security defaults
        await cookieManager.SetCookieAsync(
            httpContext, "session_id", sessionId)
            .ConfigureAwait(false);
    }
}
```

**Security defaults applied automatically:**

- `Secure = true` (HTTPS only)
- `SameSite = Lax` (CSRF protection, configurable per cookie)
- `MaxAge` calculated from `RetentionDays`
- `HttpOnly` per cookie definition

## Configuration reference

```json
{
  "Http:Cookies": {
    "ThrowOnUnregistered": true,
    "DefaultRetentionDays": 365,
    "ThirdPartyServices": [
      {
        "Name": "Google Analytics",
        "Category": "Analytics",
        "CookiePatterns": ["_ga*", "_gid"]
      }
    ]
  }
}
```

## Public API summary

| Category | Key types | Package |
| -------- | --------- | ------- |
| Modules | `GranitHttpCookiesModule`, `GranitHttpCookiesCookieConsentModule` | -- |
| Registry | `ICookieRegistry`, `CookieDefinition`, `CookieCategory`, `GranitCookiesBuilder` | `Granit.Http.Cookies` |
| Contributors | `ICookieDefinitionContributor` | `Granit.Http.Cookies` |
| Manager | `IGranitCookieManager`, `IConsentResolver` | `Granit.Http.Cookies` |
| GPC | `IGlobalPrivacyControlSignal`, `ICookieConsentModelProvider`, `ConsentModelInfo` | `Granit.Http.Cookies` |
| CMP | `UseCookieConsent()`, `CookieConsentOptions` | `Granit.Http.Cookies.CookieConsent` |
| Ledger | `CookieConsentRecord`, `IConsentLedger`, `ICookieConsentEraser`, `ConsentRecordedEto` | `Granit.Http.Cookies` |
| Ledger persistence | `AddGranitCookiesEntityFrameworkCore()`, `CookiesDbContext` | `Granit.Http.Cookies.EntityFrameworkCore` |

## See also

- [React companion: Cookies](/frontend/security/cookies/) — the `@granit` packages that consume this module from React
- [Consent & GPC](/dotnet/compliance/cookies/consent/) -- Consent models, Global Privacy Control
- [Consent Ledger](/dotnet/compliance/cookies/consent-ledger/) -- append-only server-side proof of consent, GDPR erasure
- [Klaro CMP (removed)](/dotnet/compliance/cookies/klaro/) -- migration to CookieConsent
- [Regulation Bridge](/dotnet/compliance/cookies/regulation-bridge/) -- Privacy.Regulations bridge
- [Privacy module](/dotnet/compliance/privacy/) -- Data export, erasure, legal agreements
