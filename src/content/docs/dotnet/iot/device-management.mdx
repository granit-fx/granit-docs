...
description: "Manage IoT devices in a multi-tenant .NET 10 application: DDD Device aggregate with enforced state machine, CQRS reader/writer split, Minimal API CRUD, sensitive-data scrubbing, and a PostgreSQL schema tuned for time-series telemetry (BRIN + GIN + partitioning)."
sidebar:
