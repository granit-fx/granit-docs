...
description: "High-throughput IoT telemetry ingestion for .NET 10: HMAC-SHA256 webhook validation, Redis-backed transport deduplication, Wolverine transactional outbox, and per-tenant threshold evaluation. Provider-agnostic — Scaleway IoT Hub, AWS IoT Core, and MQTT plug into the same pipeline."
sidebar:
...
For the AWS **bridge** (provisioning, Device Shadow, IoT Jobs, JITP), see
