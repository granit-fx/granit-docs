...
Signature validation, parsing, deduplication, device lookup, and the outbox
publish all run synchronously — every one must complete before the endpoint
returns `202`. Only the handler work downstream of the outbox (persist,
heartbeat update, threshold evaluation) runs asynchronously via Wolverine's
...
| `403 Forbidden` | Request blocked by an authorization policy on the route group |
| `413 Payload Too Large` | Request body exceeds the 256 KB cap |
...
| `429 Too Many Requests` | Rate-limit policy (`iot-ingest`) exceeded |

The endpoint buffers the request body up to **256 KB** (enough for even the
chattiest MQTT payload) and rejects anything larger with `413 Payload Too
Large` — an advertised `Content-Length` over the cap is refused before a
single byte is read, and chunked bodies are cut off the moment they cross it.
It then snapshots headers into a case-insensitive dictionary and delegates to
`IIngestionPipeline.ProcessAsync()`.

Rate limiting is enforced at the route group level via
`Granit.Http.RateLimiting` — the route group calls
`.RequireGranitRateLimiting("iot-ingest")` (namespace
`Granit.Http.RateLimiting.AspNetCore`) — so configure per-tenant limits under
...
<Aside type="note" title="Why fail-open?">
  ...
  reading is worse than processing a rare duplicate. Every fail-open bypass
  increments the dedicated `granit.iot.ingestion.dedup_fail_open` counter — a
  sustained non-zero rate signals a replay-window integrity gap, so page
  on-call.
</Aside>
...
| `granit.iot.ingestion.dedup_fail_open` | `tenant_id` | Redis unavailable — dedup bypassed (fail-open) |
| `granit.iot.ingestion.unknown_device` | `tenant_id`, `source` | Serial not registered |
| `granit.iot.ingestion.threshold_exceeded` | `tenant_id` | Threshold breached |
| `granit.iot.alerts.throttled` | `tenant_id` | Notification suppressed by the bridge |
...
<Aside type="caution" title="Don't skip signature validation 'just for testing'">
  ...
  insert-only database backdoor. Granit auto-registers the permissive
  `NullPayloadSignatureValidator` (source `development`) only when
  `IHostEnvironment.IsDevelopment()` is true — rely on that for local work,
  and never ship it to production.
</Aside>
