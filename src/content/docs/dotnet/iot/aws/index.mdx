...
description: "Bridge the Granit Device aggregate to AWS IoT Core — AwsThingBinding, X.509 provisioning saga, Device Shadow sync, IoT Jobs, and JITP. Six Ring-3 packages."
sidebar:
  label: AWS IoT Core
  order: 11
---
...
flowchart TB
  ...
  subgraph "AWS bridge family (Ring 3)"
    ...
    EF["Granit.IoT.Aws.EntityFrameworkCore<br/>isolated AwsBindingDbContext (iot_aws_* tables)"]
...
| `Granit.IoT.Aws.EntityFrameworkCore` | Isolated `AwsBindingDbContext` (`iot_aws_` table prefix) |
...
sequenceDiagram
  ...
  participant S as ShadowDeltaSweepJob → ShadowDeltaPollingService
  participant E as ILocalEventBus

  G->>H: DeviceActivatedEvent
  H->>AWS: UpdateThingShadow(reported = {status:"Active",...})
  Note over S,AWS: iot-aws-shadow-sync cron — every 30s, single-leader
...
- **Granit → AWS**: `DeviceLifecycleShadowHandler` reacts to
  ...
  timestamps).
- **AWS → Granit**: the `iot-aws-shadow-sync` recurring job (`ShadowDeltaSweepJob`,
  cron `*/30 * * * * *`) fires once cluster-wide via the Granit scheduler, and its
  sweep body `ShadowDeltaPollingService` walks up to `PollBatchSize` (default 100)
  active bindings per run, parses `state.delta`, and publishes
  `DeviceDesiredStateChangedEvent` on `ILocalEventBus`.
...
sequenceDiagram
  ...
  participant DB as iot_aws_* tables
...
The `iot-aws-claim-cert-rotation-check` recurring job (`ClaimCertificateRotationJob`,
hardcoded cron `0 2 * * *`) fires once cluster-wide via the Granit scheduler; its
sweep body `ClaimCertificateRotationCheckService` surfaces expiring claim
certificates as `ClaimCertificateExpiringEvent` so operators rotate them before
fleet enrolments break.
...
app.MapGranitIoTAwsFleetProvisioningEndpoints("iot-fleet-provisioning");
```

`authorizationPolicyName` is **required**. It authenticates the customer's AWS
Lambda and drives the cross-check between the JITP payload's `TenantId` and the
authenticated principal — the only line of defence against a compromised Lambda
enrolling Things into another tenant. A null or whitespace policy name throws at
startup (during `ValidateOnStart`), not at first request.

Or, with the meta-bundle — an extension on `GranitBuilder`, wired in one call
(the isolated EF Core context takes its connection string from the shared Granit
persistence configuration, so there is no per-call options delegate):
...
| `IoT:Aws:Shadow` | `PollBatchSize` (default 100), `AutoPushLifecycleStatus` (default true) |
| `IoT:Aws:Jobs` | `JobIdPrefix`, `JobTrackingTtlHours`, `StatusPollBatchSize` |
| `IoT:Aws:FleetProvisioning` | `ExpiryWarningWindowDays` (default 30), `RotationCheckBatchSize` (default 1000) |

Sweep cadences are **not** options. The Shadow, Jobs and rotation sweeps run as
recurring cron jobs on the Granit scheduler — `iot-aws-shadow-sync` (every 30s),
`iot-aws-jobs-status-poll` (every 5 min) and `iot-aws-claim-cert-rotation-check`
(daily at 02:00) — so the `*BatchSize` knobs above bound how much each single-leader
run scans, not how often it fires.
...
<Aside type="note" title="Sweep jobs vs IoT Rule">
  The Shadow and Jobs sweeps run as single-leader recurring jobs — one
  scheduled run scans the whole cluster's active bindings, so they hold up to
  a few thousand devices comfortably. Larger fleets should switch to AWS IoT
  Rule → SNS / SQS — the bridge is wired through `ILocalEventBus.PublishAsync`,
  so dropping in an event-driven consumer is mechanical.
</Aside>
