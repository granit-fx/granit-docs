...
description: "Wire Granit.IoT into a .NET 10 application in under five minutes: install Granit.Bundle.IoT, point Scaleway IoT Hub at the webhook, provision your first device, and query its telemetry. End-to-end with PostgreSQL, Redis, and Wolverine outbox."
sidebar:
...
1. ### Configure the Granit NuGet feed
   ...
   personal access token (PAT) that has the `read:packages` scope:
...
2. ### Install the bundle
   ...
   That's 11 packages — domain, EF Core persistence, PostgreSQL migrations,
   device CRUD endpoints, ingestion pipeline, Scaleway provider, Wolverine
   handlers, notifications bridge, background jobs, timeline bridge, and the
   MCP bridge.

   <Aside type="note" title="MQTT is opt-in">
     `Granit.IoT.Mqtt` and `Granit.IoT.Mqtt.Mqttnet` are not included in
     `Granit.Bundle.IoT`. Add them only if you consume from a non-Scaleway MQTT
     broker — see [MQTT transport](/dotnet/iot/mqtt/).
   </Aside>
...
3. ### Register the modules
   ...
   `AddIoT()` enumerates the 11 modules in topological order. Actual DI order
   is driven by each module's `[DependsOn(...)]` graph — you can't accidentally
   skip a dependency.
...
5. ### Apply migrations
   ...
   production insert — see [Operations](/dotnet/iot/operations/).
