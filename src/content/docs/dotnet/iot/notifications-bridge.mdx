...
description: "Route IoT threshold breaches and device-offline events through your existing multi-channel notification pipeline. Granit.IoT.Notifications maps integration events to Granit.Notifications notification types with per-tenant throttling, multi-channel routing (Email / Push / SMS), and cascaded settings — no second notification schema."
sidebar:
