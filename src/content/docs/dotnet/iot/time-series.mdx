...
description: "Scale IoT telemetry from thousands to billions of rows with two opinionated backends: PostgreSQL native (BRIN + GIN + monthly RANGE partitioning) by default, or opt-in TimescaleDB hypertables with hourly and daily continuous aggregates. Decision tree, indexes, partitioning helpers, and the dashboard query speed-up explained."
sidebar:
...
| AWS RDS PostgreSQL | ❌ Not available — use Timescale Cloud or RDS + self-managed |
