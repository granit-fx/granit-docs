...
2. `SELECT create_hypertable('iot_telemetry_points', 'RecordedAt',
   ...
   migrate_data => TRUE)` — safe on empty and populated tables, and a no-op
   if the table is already a hypertable.
3. `CREATE MATERIALIZED VIEW iot_telemetry_hourly WITH
   (timescaledb.continuous) AS …` — expand the JSONB `Metrics` column via
   `LATERAL jsonb_each`, one row per metric per hour bucket, filtered to
   `WHERE jsonb_typeof(metric.value) = 'number'` so only numeric metrics are
   aggregated.
