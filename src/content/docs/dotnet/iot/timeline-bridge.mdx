...
description: "Device lifecycle events become Granit.Timeline system-log entries — ISO 27001 asset traceability and GDPR Art. 30 records-of-processing with zero audit code."
sidebar:
...
builder.Services
    ...
    .AddModule<GranitIoTTimelineModule>();
```
...
`/timeline/{entityType}/{entityId}` by default (the route prefix is
configurable; a host may mount it behind its own `/api` group):

```bash
curl "https://your-app/timeline/Device/$DEVICE_ID" \
