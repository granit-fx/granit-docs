...
description: "Install the Granit.IoT stack via one NuGet ref and builder.AddIoT(). Two meta-bundles: cloud-agnostic core (12 pkgs) and AWS IoT Core bridge (6 pkgs)."
sidebar:
...
builder.Services
    ...
    .AddGranitIoTEntityFrameworkCore(                // persistence
        configureShared: o => o.UseNpgsql(connectionString));

// The Endpoints layer ships as a module — register it, then map its routes:
builder.AddGranit(granit => granit
    .AddModule<GranitIoTEndpointsModule>());     // CRUD API

app.MapGranitIoTEndpoints();
...
| 2 | `Granit.IoT.Aws.EntityFrameworkCore` | Isolated `AwsBindingDbContext` (`iot_aws_*` table prefix) |
