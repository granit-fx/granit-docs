...
description: "Complete database reference for Granit.IoT: ER diagram of the iot_* and iotaws_* schemas, column-by-column tables for iot_devices, iot_telemetry_points, and iotaws_thing_bindings, JSONB metric conventions, secrets-at-rest model, migration ordering with two isolated DbContexts, and sizing guidelines from thousands to billions of rows."
sidebar:
  label: Data Model
  order: 3
---
...
<Aside type="note" title="Why no cross-context FK?">
  `iotaws_thing_bindings.DeviceId` references `iot_devices.Id`, but no
  ...
  using the AWS bridge can `DROP TABLE iotaws_thing_bindings` without
...
<Tabs>
  ...
  <TabItem label="Extract one metric">
    ...
    ```
    Used by `ITelemetryReader.QueryAsync` and the aggregate endpoints.
  </TabItem>
...
- `[SensitiveData]` on `DeviceCredential.ProtectedSecret` — scrubbed from
  logs, OpenAPI schemas, and `422` validation payloads by
  `Granit.Http.Sanitization`.
- Read-time `Unprotect` happens inside the credential consumer (MQTT
  bridge, ingestion validator), **never** in an API response.
- Plaintext secrets are rejected at startup if `appsettings.json` carries
  a credential without `__FROM_SECRET_STORE__` placeholder semantics —
  see [security baseline](/dotnet/security/security-overview/).
...
<Tabs>
  ...
  <TabItem label="With AWS bridge">
    ...
    # 2. AWS bridge schema (iotaws_thing_bindings)
    dotnet ef database update \
...
- [AWS IoT Core bridge](/dotnet/iot/aws/) — the saga that owns `iotaws_thing_bindings`
