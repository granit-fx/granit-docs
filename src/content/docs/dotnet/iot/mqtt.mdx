...
description: "Wire a .NET 10 SaaS application to any MQTT broker — Mosquitto, EMQX, HiveMQ, VerneMQ, Scaleway IoT Hub in MQTT mode, AWS IoT Core, or self-hosted. Handles mTLS via Vault, automatic reconnection with exponential backoff, QoS 0–2, and feeds the same ingestion pipeline used by webhooks."
sidebar:
