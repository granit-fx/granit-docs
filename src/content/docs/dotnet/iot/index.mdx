...
description: "Multi-tenant IoT platform for .NET — DDD device aggregate, signed ingestion at P99 under 1s, MQTT and AWS IoT Core, TimescaleDB. GDPR + ISO 27001 ready."
sidebar:
  label: Overview
  order: 0
---
...
| **Provider lock-in** | Each cloud is a re-integration | Pluggable `IInboundMessageParser` / `IPayloadSignatureValidator` |
...
| --- | --- | --- |
| `Granit.Bundle.IoT` | Full Ring 1 + 2 + 3 cloud-agnostic stack (12 packages, MCP included) | New projects, full IoT functionality |
| `Granit.Bundle.IoT.Aws` | AWS IoT Core device-binding family (6 packages) | Add on top of `Granit.Bundle.IoT` when targeting AWS |

MQTT (`Granit.IoT.Mqtt[.Mqttnet]`), TimescaleDB
(`Granit.IoT.EntityFrameworkCore.Timescale`), and AWS-native ingestion
(`Granit.IoT.Ingestion.Aws`) ship as opt-in extensions outside the bundles.
...
| **Secrets at rest** | `DeviceCredential.ProtectedSecret` stores caller-supplied ciphertext and is stripped from logs, OpenAPI, and validation errors by `[SensitiveData]` (`Granit.DataProtection`) |
