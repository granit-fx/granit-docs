...
`FeaturesMetrics` ships this single counter (meter `Granit.Features`) — the
resolution cascade and override mutations are traced on the
`Granit.Features` activity source rather than counted.
