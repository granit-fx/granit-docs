...
and stored in `INotificationDefinitionStore` (singleton). The [fan-out handler](/dotnet/infrastructure/notifications/fan-out-engine/)
...
- [Overview](/dotnet/infrastructure/notifications/) — fan-out engine and package structure
- [Fan-out engine](/dotnet/infrastructure/notifications/fan-out-engine/) — how preferences and definitions drive dispatch
- [Endpoints](/dotnet/infrastructure/notifications/endpoints/) — REST API exposing preferences, subscriptions, followers
- [Package conventions](/dotnet/infrastructure/notifications/conventions/) — module-local notification definition layout
- [Persistence module](/dotnet/data/persistence/) — `AuditedEntity`, `Entity`, EF Core interceptors
- [Timeline module](/dotnet/building-blocks/timeline/) — companion audit trail for entity changes
