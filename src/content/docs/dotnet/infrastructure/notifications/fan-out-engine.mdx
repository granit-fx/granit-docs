---
title: "Fan-Out Engine — Multi-Channel Dispatch"
...
The fan-out engine is the core dispatch mechanism of Granit.Notifications. A single `PublishAsync()` call produces one `DeliverNotificationCommand` per recipient per channel, after filtering through [user preferences](/dotnet/infrastructure/notifications/data-model/#notification-preferences).
...
- [Channels](/dotnet/infrastructure/notifications/channels/) — `INotificationChannel`, custom channel implementation
- [Data model](/dotnet/infrastructure/notifications/data-model/) — `NotificationPreference`, recipient resolution
- [Wolverine integration](/dotnet/infrastructure/notifications/wolverine-integration/) — durable outbox dispatch with retry
- [Configuration](/dotnet/infrastructure/notifications/configuration/) — provider options and telemetry
- [Wolverine messaging](/dotnet/infrastructure/wolverine-messaging/) — outbox pattern and tenant context propagation
- [Presence module](/dotnet/infrastructure/presence/) — suppress push channels when a recipient's presence is Offline
