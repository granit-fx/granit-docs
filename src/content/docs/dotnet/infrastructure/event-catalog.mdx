...
:::note[Open-source scope]
The catalogue below lists events emitted by modules shipped from `granit-dotnet` (open-source).
Commercial business modules — Activities, Invoicing, Subscriptions, Catalog, Documents,
Templating, Taxonomy, CustomerBalance, Tax, Payments, Parties, Metering — live in
`granit-business` (PR #2058, May 2026) and document their event surface inside that
repository.
:::
...
|-------|------|---------|------------|
| `ApiKeyCreatedEto` | Integration | `ApiKeyEntry.Create()` | ISO 27001 |
| `ApiKeyRevokedEto` | Integration | Service layer | ISO 27001 |
| `ApiKeyRotatedEto` | Integration | Service layer | ISO 27001 |
| `ApiKeyScopesUpdatedEto` | Integration | Service layer | ISO 27001 |
| `ApiKeyExpiringSoonEto` | Integration | Expiry sweep | ISO 27001 |
| `ApiKeyExpiredEvent` | Domain | Expiry sweep | ISO 27001 |
| `ApiKeyUsedEto` | Integration | Successful key authentication | ISO 27001 |
...
| Event | Type | Trigger | Compliance |
|-------|------|---------|------------|
| `IdentityUserUpdatedEto` | Integration | Identity provider webhook | — |
| `IdentityUserDeletedEto` | Integration | Identity provider webhook | GDPR Art. 17 |
| `FederatedIdentityErasedEvent` | Domain | After hard-delete in handler | GDPR Art. 17 |
...
| Event | Type | Trigger | Compliance |
|-------|------|---------|------------|
| `ImportJobCompletedEto` | Integration | Import orchestrator | — |
| `ImportJobCancelledEvent` | Domain | `ImportJob.Cancel()` | ISO 27001 |
| `ExportJobCompletedEto` | Integration | Export orchestrator | — |
| `ExportCompletedEto` | Integration | Export archive assembled and downloadable | — |
...
- [Wolverine messaging](/dotnet/infrastructure/wolverine-messaging/) — bus that dispatches domain and integration events
- [Module system](/dotnet/core/module-system/) — `IDomainEvent`, `IIntegrationEvent`, dispatcher
- [Webhooks](/dotnet/api/webhooks/) — outbound delivery of integration events
- [Persistence](/dotnet/data/persistence/) — `DomainEventDispatcherInterceptor` and the outbox
- [Notifications](/dotnet/infrastructure/notifications/) — bridge between events and user-facing channels
