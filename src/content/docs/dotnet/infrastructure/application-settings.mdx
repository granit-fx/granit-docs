---
title: "Settings \u2014 Cascading Runtime Config with Encryption & Cache"
description: Cascading settings engine with five scopes (User, Tenant, Global, Configuration, Default), AES-256 storage, distributed cache, and runtime changes.
sidebar:
  label: Settings
  order: 5
---
...
|---------|------|------------|
| `Granit.Settings` | `ISettingProvider`, `ISettingWriter`, cascading resolution | `Granit.Caching`, `Granit.Encryption`, `Granit.Users` |
...
```csharp
public class AdminService(ISettingWriter settingWriter)
{
...
`Granit.Settings` ships an `ActivitySource` (`Granit.Settings`) but no meter of
its own — setting reads and writes are traced, not counted. Cache behaviour is
observable through the FusionCache instrumentation exposed by
[`Granit.Caching`](/dotnet/data/caching/).
...
| Abstractions | `ISettingProvider` (`GetOrNullAsync`, `GetAllAsync`), `ISettingWriter` (`SetGlobalAsync`, `SetForTenantAsync`, `SetForUserAsync`) | `Granit.Settings` |
| Definitions | `SettingDefinition`, `ISettingDefinitionProvider`, `ISettingDefinitionContext`, `SettingDefinitionRegistry` | `Granit.Settings` |
...
- [React companion: Application Settings](/frontend/infrastructure/settings/) — the `@granit` packages that consume this module from React
