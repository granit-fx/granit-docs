...
description: Guaranteed cross-module events via a transactional outbox, automatic tenant/user/trace propagation, and invalid commands rejected before they ever hit a retry.
sidebar:
  label: Wolverine
  order: 8
---
...
- Granit.Wolverine/ Core: routing, context propagation, FluentValidation middleware
  ...
  - Granit.Wolverine.SqlServer SQL Server outbox, EF Core transactions
  - Granit.Wolverine.Encryption Field-level encryption of [Encrypted] properties on the wire
</FileTree>
...
| `Granit.Wolverine.SqlServer` | SQL Server transactional outbox, EF Core integration | `Granit.Wolverine`, `Granit.Persistence` |
| `Granit.Wolverine.Encryption` | Transparent encryption of `[Encrypted]` string properties in envelopes and saga state | `Granit.Wolverine`, `Granit.Encryption` |
...
graph TD
    ...
    WS[Granit.Wolverine.SqlServer] --> W
    WS --> P
    WE[Granit.Wolverine.Encryption] --> W
    WE --> E[Granit.Encryption]
```
...
|----------|-----------|---------|
| Module | `GranitWolverineModule`, `GranitWolverinePostgresqlModule`, `GranitWolverineSqlServerModule`, `GranitWolverineEncryptionModule` | — |
| Options | `WolverineMessagingOptions`, `WolverinePostgresqlOptions`, `WolverineSqlServerOptions` | — |
...
| Attributes | `[WolverineHandlerModule]`, `[CrossTenantMessage]` | `Granit.Wolverine` |
| Extensions | `AddGranitWolverine()`, `AddGranitWolverineWithPostgresql()`, `AddGranitWolverineWithPostgresqlPerTenant<T>()`, `AddGranitWolverineWithSqlServer()`, `AddGranitWolverineWithSqlServerPerTenant<T>()`, `AddInMemoryClaimCheckStore()` | — |
...
- [Persistence module](/dotnet/data/persistence/) — `DomainEventDispatcherInterceptor`, transactional outbox
- [Multi-tenancy module](/dotnet/infrastructure/multi-tenancy/) — Tenant context propagation
- [Security module](/dotnet/security/authentication/) — `ICurrentUserService` propagation
