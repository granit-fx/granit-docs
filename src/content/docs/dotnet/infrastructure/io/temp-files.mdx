...
description: "Granit.IO provides ITempFileFactory — 0600-permissioned, tenant-partitioned temp files with guaranteed kernel cleanup. Includes K8s emptyDir vs PVC guidance."
sidebar:
  label: Temp Files
  order: 10
---
...
- `MaxSizeBytes` — default `100 MiB` per file. Bump
  `IO:TempFiles:MaxSizeBytes` when generating large PDFs.
...
- [Browsing security](/dotnet/infrastructure/browsing/security/) — primary consumer.
- [`Granit.Http.UrlSafety`](/dotnet/api/url-safety/) — the second
