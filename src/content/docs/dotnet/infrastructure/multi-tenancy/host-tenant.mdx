...
description: "Separate host infrastructure (identity, audit, jobs) from tenant business data via GranitDbDefaults — configured in appsettings.json with module classification."
sidebar:
...
| QueryEngine | `ConfigureQueryEngineModule()` | Entity views per tenant (PR #1639 supersedes the legacy SavedView surface) |
...
- [Isolation Strategies](/dotnet/infrastructure/multi-tenancy/isolation/) — the three strategies and how they interact
  with host schemas
- [Cache Isolation](/dotnet/infrastructure/multi-tenancy/cache/) — tenant-scoped cache keys
