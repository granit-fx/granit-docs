...
description: "How Granit identifies host vs tenant users — the tenant_id claim is the single source of truth. Present means tenant user, absent means host user. No parallel is_host claim, no frontend inference."
sidebar:
...
See [Tenant Resolvers](./resolvers/) for the resolver pipeline and the trust
...
- [Tenant Resolvers](./resolvers/) — `JwtClaimTenantResolver`, header trust
  modes, and the resolver pipeline
- [Host vs Tenant](./host-tenant/) — schema separation and host admin
  cross-tenant access
