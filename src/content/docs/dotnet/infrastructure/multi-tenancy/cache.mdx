...
description: "Automatic FusionCache key isolation per tenant — TenantAwareFusionCache prefixes all cache keys with the current TenantId to prevent cross-tenant cache leaks."
sidebar:
...
- [Caching module](/dotnet/data/caching/) — FusionCache setup, encryption,
  Redis L2
- [Host vs Tenant](/dotnet/infrastructure/multi-tenancy/host-tenant/) — which modules are host-level vs tenant-level
- [Tenant Resolvers](/dotnet/infrastructure/multi-tenancy/resolvers/) — how the current tenant is determined
