---
title: "Localization \u2014 18 Cultures & CLDR Plurals"
description: JSON-based i18n across 18 cultures with CLDR plural rules, source-generated keys, per-tenant runtime overrides, fallback chains, and Minimal API endpoints.
sidebar:
  label: Localization
  order: 18
---
...
The system ships with 18 cultures (15 base + 3 regional — `en-GB`, `fr-CA`, `pt-BR`), so your application is
...
|-----------|------|---------|
| `:` colon | Namespace boundary (resource scope) | `Showcase:Admin:`, `Validation:Format:` |
...
| Granit validation | `Validation:` | `Validation:Builtin:NotEmpty` |
| Granit permissions | `Permission:` / `PermissionGroup:` | `Permission:Settings.Global.Read` |
| Your application | `MyApp:Admin:` | `MyApp:Admin:Users.Title` |
...
|--------|-------|-------------|
| `GET` | `/localization/overrides` | Query-engine list of overrides (`page`, `pageSize`, `sort`, `filter[...]`, `search`, `presets[...]`, `groupBy`) |
| `GET` | `/localization/overrides/meta` | Query metadata (columns, filters, sorts, presets) for dynamic UI generation |
...
`MapGranitLocalizationOverrides()` mounts the query-engine contract on `GET /`
and `GET /meta` — the legacy `?resourceName=X&cultureName=Y` lookup was removed
in Granit 0.1.0-dev (May 2026). Filter on `resourceName` / `cultureName` via
`filter[...]` parameters instead. See
...
All 18 cultures must have a JSON resource file, even if empty. The source generator
...
- [Settings, Features & Reference Data](/dotnet/infrastructure/application-settings/) -- uses localization for display names
- [Persistence module](/dotnet/data/persistence/) -- EF Core interceptors used by the override store
- [Security module](/dotnet/security/authentication/) -- permission-based access for override management
