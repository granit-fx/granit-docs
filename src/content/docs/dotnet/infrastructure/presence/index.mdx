...
`Granit.Presence` answers a single question across every Granit module that
cares: **is this user reachable right now, and through which channels?** It
blends a volatile client heartbeat with a persisted manual override
(`Available`, `Busy`, `DoNotDisturb`, `AppearOffline`) into an effective
`PresenceStatus`, and exposes that snapshot through HTTP endpoints, an
integration event, and an optional notification delivery gate that suppresses
push channels when the user does not want to be paged.
