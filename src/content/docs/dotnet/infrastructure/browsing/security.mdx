...
description: "Deny-by-default sandbox for Granit.Browsing — HTTPS-only, no private networks, forced CSP, redacted console, 30s render cap, and the five Browsing permissions."
sidebar:
  label: Security
  order: 20
---
...
| --- | --- |
| `IUrlSafetyValidator` | Scheme/host/private-net/TLD/length pre-flight; resolves DNS. See [URL safety](/dotnet/api/url-safety/). |
...
| `BrowserScriptInjectedEvent` | On `EvaluateAsync` / `AddScriptTagAsync` / `AddStyleTagAsync`. | `PageId`, `EngineName`, `Kind`, **`ScriptHash`** (SHA-256, `[SensitiveData(Hash)]`), `InjectedAt`. |
...
- Interpolated user input inside `EvaluateAsync` is template injection
  identical to `eval(user_input)`.
...
1. Shutdown disposes the pool, which calls
   `HeadlessBrowserDrainCoordinator.DrainAsync`.
2. The drain is bounded by `GranitBrowsingOptions.DrainTimeout` (default
   30 seconds) so a misbehaving page handle can never block shutdown
   indefinitely.
3. On timeout the coordinator increments
   `granit.browsing.pool.drain.timeout`, logs a warning, and force-disposes
   the browser when it implements `IAsyncDisposable`.
...
- Full guidance: [Secure Temp Files — Kubernetes deployment](/dotnet/infrastructure/io/temp-files/#kubernetes-deployment).
...
- [URL Safety](/dotnet/api/url-safety/) — the shared SSRF primitive.
- [Secure Temp Files](/dotnet/infrastructure/io/temp-files/) — `file://` PDF viewer, HAR, trace.
