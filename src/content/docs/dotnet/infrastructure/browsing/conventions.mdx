...
description: "Conventions for Granit.Browsing consumers: deny-by-default sandbox, no CSP bypass, typed Uri APIs, mandatory permissions, ITempFileFactory for file:// handoffs."
sidebar:
  label: Conventions
  order: 15
---
...
permission grant. See [Browsing security](/dotnet/infrastructure/browsing/security/).
...
[Browsing security — Permissions matrix](/dotnet/infrastructure/browsing/security/#permissions-matrix).
...
`TenantAwareHeadlessBrowser` decorates `IHeadlessBrowser` to stamp the
acquiring `ICurrentTenant.Id` onto the page metrics and the
`BrowserPageAcquiredEvent`, and — when an `IPermissionChecker` is registered —
to require `Granit.Browsing.Pages.Acquire` before handing out a page, throwing
`UnauthorizedAccessException` otherwise. Pages are also wrapped in
`PermissionAwareBrowserPage` so per-operation permissions keep applying.

Tenant identity is captured at acquisition time, so tests that fake
`ICurrentTenant` must set it *before* `AcquirePageAsync`, not after: setting it
later produces pages attributed to the wrong tenant in metrics and events.
