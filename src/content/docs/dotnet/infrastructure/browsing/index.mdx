...
description: Provider-agnostic IHeadlessBrowser with capability discovery — PDF, screenshots, network interception, tracing, HAR. Backed by PuppeteerSharp or Playwright.
sidebar:
  label: Browsing
  order: 12
---
...
`Granit.Browsing` ships a deny-by-default `IBrowserSandboxProfile` that is
...
Both providers honour the profile through native interception, wrapped by a
...
are covered in [Browsing security](/dotnet/infrastructure/browsing/security/).
...
- **Meter** `Granit.Browsing` — counters `granit.browsing.page.acquire`,
  `granit.browsing.page.release`, `granit.browsing.error`;
...
- [Conventions](/dotnet/infrastructure/browsing/conventions/) — provider registration, capability lookup, pool lifecycle
- [Security](/dotnet/infrastructure/browsing/security/) — sandboxing untrusted content, URL allow-lists, resource limits
...
- [`Granit.Documents.Renditions`](/dotnet/business/documents-renditions/) —
  consumes `IPdfViewerCapability` to rasterise PDF uploads into thumbnails
  through the rendition pipeline.
- [`Granit.Templating`](/dotnet/building-blocks/templating/) — pairs with the
  PDF capability to render HTML templates into PDF documents.
