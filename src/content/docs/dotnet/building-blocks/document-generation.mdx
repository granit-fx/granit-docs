...
description: Binary document pipeline — HTML to PDF via Granit.Browsing (Chromium), Excel via ClosedXML, and PDF/A-3b for archival and electronic invoicing.
sidebar:
...
or Excel). Templates are managed through the [Templating](/dotnet/building-blocks/templating/)
...
- Granit.DocumentGeneration/ IDocumentGenerator facade, 2-stage pipeline (text render + binary conversion)
  - Granit.DocumentGeneration.Pdf HTML→PDF via Granit.Browsing's IPdfCapability, PdfRenderOptions, PDF/A-3b
...
| `Granit.DocumentGeneration.Pdf` | HTML→PDF renderer (consumes `IPdfCapability`), `IPdfAConverter` | `Granit.DocumentGeneration`, `Granit.Browsing` |
...
| `RenderTimeoutMs` | `30000` | Maximum rendering time in ms |
| `MaxHtmlBytes` | `16777216` | Reject HTML payloads larger than 16 MiB |

:::note[Browser settings moved]
Chromium process concerns are no longer part of `DocumentGeneration:Pdf`. The
browser pool lives in `Granit.Browsing` (`Browsing` section: `MaxBrowsers`,
`MaxPagesPerBrowser`, `AcquireTimeout`, `MaxRenderDuration`, …) and the engine
specifics in the provider sections — `Browsing:PuppeteerSharp`
(`ChromiumExecutablePath`, `DisableSandbox`, `SkipChromiumDownload`,
`ExtraArgs`, `ExpectedSha256`) or `Browsing:Playwright` (`Engine`,
`ExecutablePath`, `SkipBrowserInstall`, …). See
[Browsing](/dotnet/infrastructure/browsing/).
:::
...
- [Templating module](/dotnet/building-blocks/templating/) -- Scriban pipeline, enrichers, resolvers, template lifecycle
- [Browsing module](/dotnet/infrastructure/browsing/) -- Headless-browser abstraction (PuppeteerSharp / Playwright) consumed by the PDF renderer for HTML → PDF
...
- [Blog: HTML-to-PDF with PuppeteerSharp + Scriban templates](/blog/html-to-pdf-puppeteersharp-scriban-templates/) -- the rendering pipeline behind this module, in depth
