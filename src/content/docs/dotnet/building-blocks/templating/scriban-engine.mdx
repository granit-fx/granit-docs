...
The `app.*` context is bound to the `Templating:App` configuration section:
...
- [Overview](/dotnet/building-blocks/templating/) — package structure and setup
- [Rendering Pipeline](/dotnet/building-blocks/templating/rendering-pipeline/) — where the Scriban engine runs in the pipeline
- [Layouts](/dotnet/building-blocks/templating/layouts/) — two-pass rendering and shared envelope templates
- [Post-Render Transformers](/dotnet/building-blocks/templating/post-render-transformers/) — HTML processing after Scriban
- [MJML Email Templates](/dotnet/building-blocks/templating/mjml-email-templates/) — Scriban + MJML composition
- [Configuration & API Reference](/dotnet/building-blocks/templating/reference/) — sandbox options and engine knobs
