...
description: Type-safe finite state machine on EF Core — fluent definitions, permission guards, Odoo-style approval routing, INSERT-only ISO 27001 audit.
sidebar:
  label: Workflow
  order: 15
---
...
<FileTree>
- Granit.Workflow.Abstractions/ Pure contracts: `IWorkflowDefinition`, `WorkflowTransition`, `TransitionContext`, `TransitionResult`
  - Granit.Workflow/ Core FSM engine, fluent builder, `IWorkflowManager<TState>`, domain events
    ...
    - Granit.Workflow.AI AI-assisted transition advisor + risk-based auto-approval
...
|---------|------|------------|
| `Granit.Workflow.Abstractions` | Pure contracts (`IWorkflowDefinition`, `WorkflowTransition`, `TransitionContext`, `TransitionResult`) — split out in PR #1603 so modules can declare workflow shape without depending on the runtime | `Granit` |
| `Granit.Workflow` | FSM runtime, fluent `WorkflowDefinitionBuilder`, `IWorkflowManager<TState>`, domain events | `Granit.Workflow.Abstractions`, `Granit.Timing` |
...
| `Granit.Workflow.AI` | Transition advisor (recommend next state), risk-based auto-approval | `Granit.Workflow`, `Granit.AI` |
...
- [Core module](/dotnet/core/module-system/) -- `IDomainEvent`, module system, `IPublishable`
- [Persistence module](/dotnet/data/persistence/) -- `AuditedEntityInterceptor`, `ApplyGranitConventions`
- [Security module](/dotnet/security/authentication/) -- `ICurrentUserService`, permission checking
- [Notifications module](/dotnet/infrastructure/notifications/) -- Notification engine, channels
- [Wolverine module](/dotnet/infrastructure/wolverine-messaging/) -- Domain event routing, transactional outbox
