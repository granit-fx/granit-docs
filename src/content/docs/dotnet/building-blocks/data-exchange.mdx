...
description: CSV and Excel import/export pipeline — guided column mapping, batch validation, background jobs for large datasets, and AI-assisted header matching.
sidebar:
  label: Data Exchange
  order: 14
---
...
- Granit.DataExchange.Abstractions/ Pure contracts: `ImportDefinition<T>`, `ExportDefinition<T>`, mapping descriptors, DTOs
  - Granit.DataExchange/ Core: import/export pipelines, fluent definitions, mapping suggestion engine, reflection-based fallback `ExportDefinition`
...
| `Granit.DataExchange` | Import/export pipelines, mapping suggestion engine, reflection-based fallback `ExportDefinition` when none is declared | `Granit.DataExchange.Abstractions`, `Granit.Timing`, `Granit.Validation` |
