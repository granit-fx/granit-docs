...
description: Turn any existing lookup source into an @mentionable type with one line. A single mentions facade fans the picker across tagged sources with lenient per-type ACL and composite type:value resolution — no new contract.
sidebar:
  label: Mentions
  order: 18
---
