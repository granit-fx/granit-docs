...
| `application/pdf` | `PdfTextExtractor` | `Granit.TextExtraction.Pdf` |
...
<Aside type="caution" title="Scanned PDFs are not rasterised (yet)">
`application/pdf` is routed to `PdfTextExtractor`, which extracts the embedded
text layer. A scanned PDF with no text layer produces empty content — it is
**not** automatically rasterised and routed through OCR. Wiring page
rasterisation between the PDF extractor and the OCR extractor is tracked under
epic [`#2233`](https://github.com/granit-fx/granit-dotnet/issues/2233).
</Aside>
...
### Body-size cap — `LimitedStream` (VULN-001)
...
### Zip-bomb gate — `OpenXmlGate` (VULN-001)
...
### Pixel-bomb gate — Tesseract OCR (VULN-001)
...
### SSRF gate — HTML extractor (VULN-300)
...
### Tika sidecar — hostname allowlist + mTLS (VULN-102)
...
### Vision OCR — AIQuota cost ceiling + GDPR Article 28 (VULN-102)
...
public sealed class CsvSummaryExtractor : ITextExtractor
{
        ...
        Stream source, string contentType, int maxCharLength, CancellationToken ct)
    {
        ...
        {
            // 2. Soft-skip parser failures — never throw.
            return new TextExtractionResult("", null, true, 0, Name);
        }
    }
}
```
...
content), and stop producing text once `maxCharLength` is reached. The
...
- **Scanned PDF rasterisation** — `application/pdf` currently extracts only the
  embedded text layer. Page-image rasterisation routed through OCR is tracked
  under epic [`#2233`](https://github.com/granit-fx/granit-dotnet/issues/2233).
- **Tika sidecar CI shard** — the `libtesseract` and Apache Tika containers
  needed for the OCR + Tika integration test shard ship as a follow-up under
  the same epic.
