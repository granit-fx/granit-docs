...
Hosts that index trusted templates (e.g. an internal CMS) opt in **in code**,
not through configuration: construct `AngleSharpHtmlToPlainTextConverter` with
`AngleSharpConfiguration.BuildForTrustedTemplates()` and register it yourself.
There is deliberately no appsettings switch — flipping SSRF protection must be a
reviewed code change, not a deploy-time toggle. The default stays SSRF-safe.
