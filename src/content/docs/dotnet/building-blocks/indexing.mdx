...
`Granit.Indexing.Embeddings` decorates both ports:

1. **Write decorator** — embeds `entry.Content` via `IEmbeddingGenerator` once and
   forwards the resulting `Embedding` to the storage backend on the **same row**
   as `Content`. (Architecture-pinned: no sidecar entity type allowed.)
2. **Read decorator** — embeds `request.Query` once per call, fetches a deep pool
   from both channels in parallel
   (`poolSize = max(RrfFetchPoolSize, (offset + limit) * 2)`), then fuses with
...
3. Slices `Skip(offset).Take(limit)` **after** the fusion. Backend-level
...
`AddGranitIndexingEmbeddingsWriter` and `AddGranitIndexingHybridSearch` MUST be
the **last** decorators applied to their target services so the embedding write
reaches the storage backend. Both fast-fail at composition time when their
dependencies are missing — no silent degraded mode. If the embedding call fails
at search time (transport, timeout), the backend degrades to lexical-only with
the original `(offset, limit)` — the user still gets results.

<Aside type="caution" title="HNSW ghost vectors — operations concern">
On Postgres + pgvector, an HNSW index keeps a pointer to deleted vectors in its
graph until a `REINDEX INDEX CONCURRENTLY` runs. Elasticsearch retains deleted
vectors until `forcemerge`. The framework cannot schedule this — it's an ops
concern. `RecommendHnswReindexCadenceDays = 7` is informational only; wire a
cron job:
...
job that iterates every key emitted by an `IIndexedEntrySource<TKey>` and calls
`IIndexer<TKey>.IndexAsync` per entry. Use it after a new tokenizer / analyzer
deployment, after onboarding a new tenant who needs a backfill, or after a
...
### Authorization — host-enforced at dispatch

`RebuildIndexJob` is on-demand and carries no `[RecurringJob]` attribute. Hosts
that want a recurring rebuild wire their own cron-driven trigger that emits the
job.

<Aside type="caution" title="Permission check at dispatch, not in the handler">
...
envelope. **The permission check MUST live at the dispatch site** — otherwise
any in-process code path that obtains `IBackgroundJobDispatcher` can rebuild
any tenant.
...
<TabItem label="Add hybrid (RRF) retrieval">

```csharp
builder.Services.AddSingleton<IEmbeddingGenerator<string, Embedding<float>>>(sp =>
    sp.GetRequiredService<IGranitAIWorkspaceStore>()
      .Resolve("default")
      .GetEmbeddingGenerator());

builder.Services.AddGranitIndexingEmbeddingsBackend<Guid, MyHitResponse>(
    row => new MyHitResponse(row.Key, row.Summary ?? string.Empty, row.Tags));
builder.Services.AddGranitIndexingEmbeddings();
builder.Services.AddGranitIndexingEmbeddingsWriter<Guid>();
builder.Services.AddGranitIndexingHybridSearch<Guid, MyHitResponse>();
```
...
Wire a weekly `REINDEX INDEX CONCURRENTLY` on the HNSW index — see
[HNSW ghost vectors](#hybrid-retrieval--reciprocal-rank-fusion).
