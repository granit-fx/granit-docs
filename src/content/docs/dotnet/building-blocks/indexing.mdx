---
title: "Full-Text & Semantic Search — Postgres & Elasticsearch"
...
new infrastructure for a Granit host. **Elasticsearch 9.x** is opt-in
...
| tsquery / Lucene injection from anonymous traffic | `plainto_tsquery` / `simple_query_string` (restricted flags) on the default path; advanced syntax gated on a host-defined permission |
...
- Granit.Indexing/ Contracts: `IIndexer<TKey>`, `ISearchService<TKey, TResult>`, `ISearchBackend<TKey, TResult>`, `ISearchResultAuthorizer<TKey>`, `IIndexedEntrySource<TKey>`, `IIndexedDataEraser`, `IndexedEntry<TKey>`, `SearchPage<TResult>`
  ...
  - Granit.Indexing.Elasticsearch/ ES 9.x backend, `Shared` / `PerTenant` strategies, restricted `simple_query_string`
...
| `Granit.Indexing.Elasticsearch` | ES 9.x backend — BM25 multi-field per-language analyzers, `Shared` / `PerTenant` strategy, `delete_by_query` eraser | `Granit.Indexing`, `Elastic.Clients.Elasticsearch` |
...
Lucene `query_string` on ES) — endpoints MUST gate this behind a
host-defined permission (for example `Search.Advanced.Execute`; the framework
ships no such permission) before forwarding the flag.
...
| Concern | Postgres `tsvector` | Elasticsearch 9.x |
|---------|---------------------|--------------------|
