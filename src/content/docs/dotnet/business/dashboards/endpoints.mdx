---
title: "Dashboards Endpoints — Catalogue, CRUD, Widgets"
description: "Granit.Dashboards HTTP surface — browse the DashboardDefinition catalogue, import into tenant Dashboards, CRUD and publish/archive/restore, manage widgets."
sidebar:
  label: Endpoints
  order: 5
---
...
Enum values travel as PascalCase strings on the wire (ADR-039 §6.1) — `RefreshHint`, `WidgetSnapshotStatus`, `MetricValueKind`, etc. all serialise that way. Frontend TypeScript types match exactly: `type RefreshHint = "Static" | "Dynamic" | "Realtime"`. The database side mirrors the wire — see [Enum Persistence](/dotnet/data/enum-persistence/) for the storage convention.
...
Points come from one of three sources, resolved per widget:

- **LatLng** — `double` / `decimal` lat-lng columns. Works on any database, zero extra packages.
- **PostGIS** — a `geography(Point)` column. Opt-in via the `Granit.Analytics.PostGIS` package; without it the renderer surfaces `Widget:Unavailable.MapGeographyNotImplemented`.
- **`address`** — a **postal-address** column with no stored coordinates. Requires a registered [`IGeocodingService`](/dotnet/building-blocks/geocoding/) — add `Granit.Geocoding` plus a provider (`Granit.Geocoding.Nominatim` or `Granit.Geocoding.Photon`). The renderer geocodes each row's address through that service (cache included) and emits the same `points` shape. Rows that don't resolve are **dropped** from the marker set, not failed — one bad street name never aborts the widget. With no `IGeocodingService` registered, an `address`-sourced widget surfaces `Widget:Unavailable.GeocodingNotConfigured`.

<Aside type="caution" title="The `address` source geocodes personal data">
A postal address is personal data. The `address` source sends each row to the
configured geocoder, so deploy [Granit.Geocoding](/dotnet/building-blocks/geocoding/)
against a **self-hosted** Nominatim to keep PII in your boundary, and never log
the resolved addresses. A third-party geocoder needs a lawful basis, a DPA, and a
Article 30 register entry — see [Privacy and GDPR](/dotnet/building-blocks/geocoding/#privacy-and-gdpr).
</Aside>
...
- [Permissions](https://gitlab.digitaldynamics.be/digital-dynamics/granit-fx/granit-business/-/blob/develop/src/Granit.Dashboards.Abstractions/DashboardsPermissions.cs) — full source of the permission constants.
