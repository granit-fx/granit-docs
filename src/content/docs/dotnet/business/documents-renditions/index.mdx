...
**Async generation (default).** `Granit.Documents.Renditions.Wolverine`
...
before calling `AddGranitDocumentsRenditionsWolverine()`.
...
- [Documents — Asset Metadata](/dotnet/business/documents-asset-metadata/) —
  ...
  independently.
- [Documents — Public Links](/dotnet/business/documents-public-links/) —
  token-signed anonymous URLs for sharing the original or a rendition with
  unauthenticated clients (TTL, max-uses, revocation, audit).
