...
description: Multilingual lookup tables for countries, currencies, and custom statuses — 17-culture labels, Metadata, hierarchies, EF Core seeding, generic CRUD.
sidebar:
...
currencies, document types). Each entity has a unique `Code`, 15 language labels
(covering all 18 framework cultures — the 3 regional variants fall back to their
base language), automatic label resolution based on `CurrentUICulture`, and an
extensible `Metadata` JSON bag for application-specific fields.

Every reference-data type registered through `AddReferenceDataStore<T, TCtx>`
is automatically exposed as a `Granit.QueryEngine` lookup source (PR #1128) —
no extra `IDataLookupSource` plumbing required.
    ...
    : AuditedEntity, IActive, IHasMetadata, IEmitEntityLifecycleEvents
{
    ...
    //     LabelZh, LabelJa, LabelPl, LabelTr, LabelKo, LabelSv, LabelCs, LabelHi
...
`search=belg`, `page=1`, `pageSize=20`. See [QueryEngine](/dotnet/building-blocks/query-engine/)
for full syntax.
...
- [React companion: Reference Data](/frontend/business/reference-data/) — the `@granit` packages that consume this module from React
