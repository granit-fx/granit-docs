...
description: Token-signed anonymous URLs for Granit.Documents — TTL, max-uses, revocation, rate limiting, SIEM consumption events. Token never persisted or logged.
sidebar:
...
Three packages, mirroring the Documents / Renditions / Properties split:
...
| GPS / PII in served bytes | Out of scope — see [Documents — Properties](/dotnet/business/documents-properties/) for the synchronous GPS and personal-data scrubs on upload. |
...
- [Documents — Properties](/dotnet/business/documents-properties/) —
  GPS / personal-data scrubs and descriptive-metadata extraction.
