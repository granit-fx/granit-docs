...
description: Odoo res.partner-style aggregate for customers, suppliers, employees, and leads — multi-role flags, parent-child hierarchy, external mappings, tax class.
sidebar:
  label: Parties
  order: 20
---
...
- Granit.Parties/ Aggregate root, domain events, query / export definitions
  ...
  - Granit.Parties.Mergeable Merge adapter + Party-children rewriter (registers with Granit.EntityMerge)
...
| `Granit.Parties.Mergeable` | Merge adapter + Party-children / Party-parent reference rewriters | `Granit.Parties.EntityFrameworkCore`, `Granit.EntityMerge.EntityFrameworkCore` |
...
`Granit.EntityMerge` framework. A merge folds a **loser** Party into a **survivor**:
...
- [Bank Accounts](/dotnet/business/bank-accounts/) — the account referential owned by a Party (`res.partner.bank`)
