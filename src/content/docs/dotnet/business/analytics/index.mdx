...
description: "Granit.Analytics — declarative MetricDefinition, composable dashboards, and an OData v4 feed for Power BI, Excel, and Tableau. Multi-tenant safe."
sidebar:
  label: Overview
  order: 0
---
...
- Layer 1 — Inline metrics  KPIs above a list, single-value tiles
  - Granit.Analytics.Abstractions  Declarative MetricDefinition, MetricValueKind, IWidgetSource
  - Granit.Analytics  Registry, FusionCache plumbing, period resolver
  - Granit.Analytics.EntityFrameworkCore  EF Core executor with empty-set semantics
  - Granit.Analytics.Endpoints  POST /metrics/:name surface
- Layer 2 — Composable dashboards  Drag-drop layouts, multiple widget kinds
  - Granit.Dashboards.Abstractions  DashboardDefinition, RefreshHint, widget contracts
  - Granit.Dashboards  Registry + push primitives
  - Granit.Dashboards.EntityFrameworkCore  Persisted Dashboard aggregate
  - Granit.Dashboards.Endpoints  REST surface
  - Granit.Dashboards.Push / .Push.WebSockets  Server-push delivery
- Layer 3 — OData v4 connector  Power BI / Excel / Tableau native consumption
  - Granit.Http.ODataExposure  Bridge QueryDefinition → OData EntitySet (ships in granit-dotnet)
</FileTree>

## Package structure (Layer 1, shipped)

<FileTree>
- Granit.Analytics.Abstractions  Pure abstractions — no EF Core, no FusionCache
...
- Granit.Analytics  Registry, FusionCache plumbing
