...
description: "Connect Power BI, Excel, or Tableau to a Granit app via the OData v4 feed from Granit.Http.ODataExposure — tenant-isolated, permission-gated, rate-limited."
sidebar:
...
1. **Download the connector descriptor.** The
   ...
   The path follows the framework's `Granit.Http.ApiDocumentation`
...
<Aside type="tip" title="Manual connection (no .pbids)">
  *Get Data → OData feed → Basic*, paste
  ...
  the host's `Granit.Http.ApiDocumentation` exposes), then *OK → Sign in
...
`.ExpandWhitelist("Customer")`. The whitelist is per EntitySet, not global, and
accepts **dotted paths** for nested navigations (`.ExpandWhitelist("Lines.Product")`
also permits `Lines`); each level counts against `MaxExpansionDepth` and every
reachable type needs a registered `ExportDefinition`. Ask the application team
which navigations are exposed.
...
### `$filter` is translated, not passed through

`$filter` is translated to the framework's `QueryPredicate` and enforced by the
same engine as the application's grids — a column must be `.Filterable()` in the
`QueryDefinition`, and `$orderby` targets must be `.Sortable()`, or the request
returns `400`. Plain comparison and boolean logic fold cleanly; two Power BI
edges to know about:

- **Case-insensitive text filters** fold to `tolower(...)`, which is rejected.
  Model text columns with a consistent case, or filter client-side.
- **Date-hierarchy drill-downs** can fold to `year(...)`/`month(...)`, also
  rejected. Use explicit date-range filters instead
  (`IssuedOn ge 2026-01-01 and IssuedOn lt 2027-01-01`).

A rejected fold does not break the report — Power BI falls back to client-side
filtering after the `400`, transferring more rows. See the full supported /
rejected tables in [OData Exposure](/dotnet/api/odata-exposure/#filter--translated-to-querypredicate).
