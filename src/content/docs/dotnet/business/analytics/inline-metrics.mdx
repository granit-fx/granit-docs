...
description: "Ship a Granit.Analytics MetricDefinition end-to-end — declare, register, localize, and consume from a React MetricCard with caching and period comparison."
sidebar:
...
1. **Add a project reference** to `Granit.Analytics` in the module's `.csproj`,
...
2. **Mark the dependency** on the module class:
...
3. **Register the metric** in your `AddGranit{Module}` extension:
...
4. **Ship localization keys** for the metric title across the 18 cultures
...
5. **Wire the host** to expose the HTTP endpoint. Once your module is loaded
...
6. **Consume from the frontend** with the `<MetricCard>` component (story
...
(the `{version}` segment is supplied by the host's `Granit.Http.ApiDocumentation`
