...
description: "Naming and structural conventions for Granit.Analytics MetricDefinition, dashboards, and period tokens — aligned with DAX (Power BI, Analysis Services)."
sidebar:
  label: Conventions
  order: 10
---
...
| ------- | ------ | ------- |
| File path | `src/Granit.{Module}/Metrics/{MetricName}MetricDefinition.cs` | `src/Granit.Invoicing/Metrics/UnpaidInvoiceCountMetricDefinition.cs` *(granit-business)* |
...
- **Pairing rule** (D1 #1396): every dashboard widget references a metric or query
  ...
  usable.
- **Map point source is declarative**: a `Map` widget sources its markers from
  lat/lng columns, a PostGIS `geography(Point)` column, or an `address` column
  geocoded through a registered [Granit.Geocoding](/dotnet/building-blocks/geocoding/)
  `IGeocodingService` (Nominatim or Photon provider). The `address` source sends
  personal data to a geocoder, so prefer a self-hosted instance and never log the
  addresses — see [Dashboards endpoints — Map](/dotnet/business/dashboards/endpoints/#map--geocoded-markers).
