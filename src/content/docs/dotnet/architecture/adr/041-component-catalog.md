...
- [ADR-040](/dotnet/architecture/adr/040-three-tier-metadata-architecture/) — Three-tier metadata architecture. Establishes that component choice is a Tier A concern (compiled), not Tier B (runtime customization).
- [ADR-042](/dotnet/architecture/adr/042-view-catalog/) — View catalog. Same closed-set + namespaced-extension pattern, applied to collection display strategies (List, Kanban, Calendar, …).
- [ADR-047](/dotnet/architecture/adr/047-entity-view/) — `EntityView`. User-saved views may not change a collection's `kind`, mirroring the rule that user-saved field state may not change a component.
