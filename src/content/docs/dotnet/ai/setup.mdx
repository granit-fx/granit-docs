...
description: Provider-agnostic AI for .NET on Microsoft.Extensions.AI. Swap OpenAI, Azure OpenAI, Ollama per env. Multi-tenant workspaces, quotas, ISO 27001 audit.
sidebar:
  label: Setup
  order: 2
  badge:
...
(default). Tracking is **automatic**: the chat-client factory wraps every client in a
`UsageTrackingChatClient` delegating middleware, so a record is stamped for you on each
model call — **no consumer stamps usage by hand**, and no code path can forget to. The
`AIUsageRecord` captures:
...
| `ConversationId` | Agentic-chat conversation the call belongs to (`Guid?`, `null` for non-chat calls) |
...
Because the middleware sits **below** `FunctionInvokingChatClient`, it stamps **one record
per model round-trip**: an agentic turn that calls the model, runs a tool, and calls the
model again produces **N records for N round-trips**, all sharing the run's
`ConversationId`. Sum them for the turn's true cost.
...
### Per-conversation slicing

When the call originates from the [agentic chat surface](/dotnet/architecture/adr/067-agentic-chat-tool-registry-prompt-catalog/),
`ConversationId` links the usage record to its conversation, so tokens, latency, and
cost can be billed or audited per conversation. It is filterable and group-by enabled
on the [`GET /ai/usage`](/dotnet/ai/endpoints/) query. Three constraints apply:

- **No migration ships with the framework.** `ConversationId` is a new column on
  `ai_usage_records`; the consuming application owns the EF migration that adds it.
- **Not an OpenTelemetry tag.** `ConversationId` is deliberately excluded from the
  metric tags — its cardinality is unbounded. Per-conversation slicing is a relational
  query, never a metric dimension.
- **No per-message linkage yet.** Each round-trip already stamps its own record (so a turn
  is several records sharing one `ConversationId`), but records attach to the conversation,
  not to an individual message. Per-message (`MessageId`) linkage is a planned follow-up.
