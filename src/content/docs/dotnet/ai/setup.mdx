...
Three providers ship in-box. They share the same `IAIProviderFactory` contract — pick
...
<Aside type="note">
Anthropic, Mistral, and other providers compatible with `Microsoft.Extensions.AI` can
be plugged in by implementing `IAIProviderFactory` (and optionally `IAIModelCatalog`).
Only the three above ship as first-party packages.
</Aside>
