...
description: AI transition advisor and risk-based auto-approval for Granit.Workflow — the LLM suggests the next step and scores risk; humans stay in control.
sidebar:
  label: Workflow Decision Support
  order: 7
  badge:
    text: New
    variant: tip
...
given the current entity state and a serialized context (entity fields plus the
    ...
    IAITransitionAdvisor advisor)
{
        ...
        CancellationToken ct)
    {
        ...
        string entityContext = JsonSerializer.Serialize(new
        {
            invoice.Id,
            invoice.Amount,
            invoice.DueDate,
            ...
            AllowedTransitions = allowed.Select(t => t.ToState.ToString()),
        });
    ...
    IAIApprovalEvaluator evaluator)
{
    ...
    {
        string entityContext = JsonSerializer.Serialize(evt.InvoiceSnapshot);
...
- [Workflow](/dotnet/building-blocks/workflow/) — the workflow module
