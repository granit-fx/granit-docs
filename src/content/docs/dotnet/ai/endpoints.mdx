---
title: "AI Endpoints — Workspaces, Models, Usage, Inference"
description: Granit AI Minimal APIs — workspace CRUD, model discovery, usage analytics, chat/embedding proxy with SSE streaming. Permission-scoped, RFC 7807.
sidebar:
  label: Endpoints
  order: 3
  badge:
...
`ConversationId` (`Guid?`, `null` for non-chat calls) is the relational path for
...
owns the EF migration that adds it. See [Usage tracking](/dotnet/ai/setup/#per-conversation-slicing).
