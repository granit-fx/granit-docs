...
description: AI log analysis for Granit.Observability — send batches of structured log entries to the LLM and get human-readable summaries with categorized insights.
sidebar:
  label: Log Analysis
  order: 14
  badge:
...
```csharp
// Declarative registration — Granit.BackgroundJobs discovers the attribute.
[RecurringJob("0 * * * *", "observability-hourly-log-analysis")]  // Every hour
public sealed record HourlyLogAnalysisJob : IBackgroundJob;
