...
description: AI audit-trail summarization and anomaly detection — collapse hundreds of timeline entries into a readable summary and surface unusual patterns.
sidebar:
  order: 9
  badge:
    text: New
...
```json
{
  "AI": {
    "Timeline": {
      ...
      "TimeoutSeconds": 30,
      "SummarizerMaxEntries": 200,
      "AnomalyDetectorMaxEntries": 500
    }
  }
}
```
...
(up to `SummarizerMaxEntries`, default 200), sends them to the LLM, and returns a natural language
summary:
...
The detector uses `AnomalyDetectorMaxEntries` (default 500) to cap the context window for the LLM — higher than the summarizer cap because anomaly signal improves with volume.
...
| `SummarizerMaxEntries` | `int` | `200` | Max entries fed to the summarizer LLM (token-budget bound) |
| `AnomalyDetectorMaxEntries` | `int` | `500` | Max entries fed to the anomaly-detection LLM (signal-to-noise bound) |
...
- [Timeline](/dotnet/building-blocks/timeline/) — the audit trail module
