...
description: FluentValidation with auto endpoint validation, structured error codes, OpenAPI schema enrichment, and validators for IBAN, BIC, E.164, ISO 3166.
sidebar:
  label: Validation
  order: 6
---
...
| `Granit.Validation.NorthAmerica` | US/Canada regulatory validators (SSN, EIN, SIN, ITIN) | `Granit.Validation`, `Granit.Localization` |
| `Granit.Validation.UnitedKingdom` | UK regulatory validators (NI Number, NHS Number, UTR) | `Granit.Validation`, `Granit.Localization` |
...
```json
{
  ...
  "errors": {
    ...
    "Phone": ["Validation:Format:E164Phone"],
    "Iban": ["Validation:Format:Iban"]
  }
}
```
...
| Custom (IBAN, E.164...) | `x-granit-validator: "Validation:Format:Iban"` |
...
{
  "pattern": "^[A-Z]{2}$",
  "x-granit-pattern-hint": "Validation:Hint:Alpha2Code"
}
```
...
| Key | en | fr |
| --- | -- | -- |
| `Hint:Alpha2Code` | 2 uppercase letters (e.g. FR, BE) | 2 lettres majuscules (ex. FR, BE) |
| `Hint:Alpha3Code` | 3 uppercase letters (e.g. FRA, BEL) | 3 lettres majuscules (ex. FRA, BEL) |
| `Hint:NumericCode` | 3 digits (e.g. 250, 056) | 3 chiffres (ex. 250, 056) |

All hint keys are prefixed with `Validation:` (omitted in the table).
...
| -------- | --------- | ------ | ---------- |
| Party | `.Email()` | RFC 5321 practical subset | `Format:Email` |
| Party | `.E164Phone()` | `+` followed by 7-15 digits | `Format:E164Phone` |
| Payment | `.Iban()` | ISO 13616 (MOD-97 check) | `Format:Iban` |
| Payment | `.BicSwift()` | ISO 9362 (8 or 11 chars) | `Format:BicSwift` |
| Payment | `.SepaCreditorIdentifier()` | EPC262-08 (MOD 97-10) | `Format:SepaCreditorIdentifier` |
| Locale | `.Iso3166Alpha2CountryCode()` | 2 uppercase letters | `Format:Iso3166Alpha2` |
| Locale | `.Bcp47LanguageTag()` | `fr`, `fr-BE`, `zh-Hans-CN` | `Format:Bcp47LanguageTag` |

All error codes are prefixed with `Validation:` (omitted in the table for brevity)
— see [ADR-068](/dotnet/architecture/adr/068-validation-key-naming-convention/)
for the `Validation:{Category}:{Rule}` shape.
...
// POST /validation/validate-batch
{
  "fields": [
    { "errorCode": "Validation:Format:Iban", "value": "BE68539007547034" },
    { "errorCode": "Validation:Format:Email", "value": "bad" },
    { "errorCode": "Unknown:Code", "value": "x" }
  ]
}
// → { "results": [
//     { "errorCode": "Validation:Format:Iban", "status": "Valid" },
//     { "errorCode": "Validation:Format:Email", "status": "Invalid" },
...
The package has **no dependency** on `Granit.RateLimiting` or `Granit.Bulkhead`.
...
- [Blog: Validation at the right layer](/blog/validation-at-the-right-layer/) -- where to put validation and why Granit splits it across layers
