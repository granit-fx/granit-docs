...
description: Granit module system with topological [DependsOn] loading, DDD base types, data filtering, domain events, and shared exceptions — the framework foundation.
sidebar:
  label: Module System
  order: 1
  badge:
...
sequenceDiagram
    ...
    participant M1 as GranitTimingModule
...
```csharp
public class GranitNotificationsSmtpModule : GranitModule
{
    public override bool IsEnabled(ServiceConfigurationContext context)
    {
        return context.Configuration.GetValue<bool>("Granit:Notifications:Smtp:Enabled");
    }
}
```
...
- [Module system concept](/dotnet/concepts/module-system/)
- [Persistence](/dotnet/data/persistence/) — EF Core interceptors that implement audit and soft delete
- [Security](/dotnet/security/authentication/) — `ICurrentUserService` that populates `CreatedBy` / `ModifiedBy`
- [Multi-tenancy](/dotnet/infrastructure/multi-tenancy/) — Full tenant isolation implementation
