---
title: "Diagnostics — Kubernetes Health Probes"
description: Kubernetes /health/live, /health/ready, /health/startup endpoints with stampede-protected caching and Grafana-friendly JSON. Survives load spikes.
sidebar:
  label: Diagnostics
  order: 5
---
...
| `Granit.Notifications.Smtp` | `AddGranitSmtpHealthCheck()` | EHLO handshake via MailKit | readiness |
| `Granit.Notifications.AwsSes` | `AddGranitAwsSesHealthCheck()` | `GetAccount()` (Degraded if sending paused) | readiness, startup |
...
| `Granit.Notifications.AzureCommunicationServices` | `AddGranitAcsEmailHealthCheck()` | Non-mutating config/credential validation | readiness |
| `Granit.Notifications.AzureCommunicationServices` | `AddGranitAcsSmsHealthCheck()` | Non-mutating config/credential validation | readiness |
| `Granit.Notifications.AzureNotificationHubs` | `AddGranitAzureNotificationHubsHealthCheck()` | Hub description | readiness |
