...
description: High-throughput usage events with idempotency dedupe, watermark aggregation, quota enforcement with alerts, and billing integration events. API or IoT.
sidebar:
  label: Metering
  order: 4
---
...
Lifecycle transitions are one-directional: `Publish()` moves `Draft → Published`,
`Archive()` moves `Published → Archived`. Only `Published` meters aggregate
production events; `Archived` meters preserve history but reject new events. Both
methods throw `InvalidOperationException` from any other source status.
...
| `LifecycleStatus` | int | `WorkflowLifecycleStatus`: Draft / Published / Archived (ADR-033) |
