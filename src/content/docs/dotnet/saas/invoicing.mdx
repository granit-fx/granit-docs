...
description: Invoices and credit notes with partial payments, overpayment detection, tax providers, gap-free numbering, and Odoo/Sage sync. For SaaS and e-commerce.
sidebar:
  label: Invoicing
  order: 2
---
...
- Granit.Invoicing/ Domain: Invoice (unified with CreditNote), line items, tax, provider abstractions
  ...
  - Granit.Invoicing.Builtin Self-hosted PDF via Granit.Templating (Phase 2)
  - Granit.Invoicing.Odoo External accounting sync via XML-RPC (Phase 2)
  - Granit.Invoicing.Notifications Email notifications for status changes (overdue, paid, ...)
</FileTree>

VAT calculation and VIES validation are delegated to the separate
[Tax module](/dotnet/saas/tax/) (`Granit.Tax`, `Granit.Tax.Builtin`,
`Granit.Tax.Stripe`) — they ship as their own package family rather than as
an Invoicing sub-package.

## Invoice lifecycle

The lifecycle is driven by a `Granit.Workflow` finite-state machine
(`IWorkflowManager<InvoiceStatus>`), which enforces transition validity,
permissions, and the ISO 27001 audit trail on every state change.
...
stateDiagram-v2
    ...
    Uncollectible --> Cancelled: Cancel()
```
...
- **Cancelled**: annulled before payment, invoice number preserved (terminal)
- **Uncollectible**: bad debt write-off, can still recover

:::caution[Cancel before payment only]
`Cancel()` is allowed only while the invoice is unpaid (Open or Uncollectible).
Cancelling an invoice that has recorded payments is **forbidden** — `Cancel()`
throws `InvalidOperationException` ("Issue a credit note instead."). To reverse
a paid invoice, issue a **credit note** instead. The invoice number is always
preserved so the fiscal sequence stays gap-free.
:::
...
| POST | `/api/{version}/invoicing/invoices` | `Invoicing.Invoices.Create` |
| POST | `/api/{version}/invoicing/invoices/{id}/finalize` | `Invoicing.Invoices.Finalize` |
| POST | `/api/{version}/invoicing/invoices/{id}/cancel` | `Invoicing.Invoices.Cancel` |
| POST | `/api/{version}/invoicing/invoices/{id}/mark-uncollectible` | `Invoicing.Invoices.MarkUncollectible` |

The transition endpoints (`finalize`, `cancel`, `mark-uncollectible`) delegate
to `IWorkflowManager<InvoiceStatus>` before applying the domain method, so they
return `409 Conflict` when the target state is unreachable from the current one.
`POST /invoices/{id}/cancel` takes an optional `CancelInvoiceRequest { Reason }`
that is persisted on the workflow transition record for the audit trail.
