...
description: Provider-agnostic tax engine — EU VAT (reverse charge, OSS, export), VIES/Stripe tax-ID validation, configurable rates. Self-hosted or Stripe Tax.
sidebar:
  label: Tax
  order: 5
---
...
- Granit.Tax/ Abstractions: ITaxIdValidator, ITaxRateProvider, domain types, metrics
  - Granit.Tax.Builtin Self-hosted EU VAT rules + VIES validation + configurable rates
...
// Granit.Tax.Builtin provides ITaxCalculator, ITaxIdValidator, ITaxRateProvider
// via GranitTaxBuiltinModule — auto-discovered by the module system.
...
Register `GranitTaxStripeModule` instead of `GranitTaxBuiltinModule` — the
