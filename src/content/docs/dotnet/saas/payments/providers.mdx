...
Two managed alternatives are available when you prefer to delegate the bank
relationship and mandate orchestration to a specialised provider:

- **`Granit.Payments.SepaDirectDebit.GoCardless`** — GoCardless API integration:
  hosted mandate sign-up, webhook-driven status reconciliation.
- **`Granit.Payments.SepaDirectDebit.Twikey`** — Twikey API integration with the
  same hosted mandate / webhook contract for SEPA-zone deployments.

The three SDD packages all implement `IPaymentProvider` with the same surface; the
runtime selector picks one based on `PaymentMethodType = "sepa_debit"` plus the
provider id you configured (`Granit:Payments:SepaDirectDebit:Provider`).

The **creditor** IBAN/BIC and the **mandate** debtor account now converge onto the
[BankAccounts referential](/dotnet/business/bank-accounts/): the per-tenant
`SepaDirectDebitConfiguration` stamps an encrypted creditor snapshot (pain.008 reads it
instead of host options), and each mandate carries a `DebtorPartyId` plus an immutable
signed IBAN snapshot. See
[SEPA configuration](/dotnet/saas/payments/sepa-configuration/) for the full model and
the migration note (creditor coordinates were removed from
`SepaDirectDebitBuiltinOptions`).
...
The **beneficiary** (collection) account is now per-tenant: the new
`Granit.Payments.SepaTransfer.EntityFrameworkCore` + `.Endpoints` packages hold a
`SepaTransferConfiguration` stamped from a [BankAccount](/dotnet/business/bank-accounts/)
on the company Party (`GET`/`PUT /sepa-transfer/configuration`). The beneficiary
coordinates were **removed** from `SepaTransferOptions`. See
[SEPA configuration](/dotnet/saas/payments/sepa-configuration/) for wiring and the
migration note.
