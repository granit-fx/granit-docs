...
- Granit.IpGeolocation.IpInfo/ ipinfo.io HTTP provider (online, no local data)
...
GeoIP2. That includes [DB-IP](https://db-ip.com/) databases, published by a Belgian (EU) company.

<Aside type="tip" title="Strictest GDPR: EU, zero third-party transfer">
For the tightest data-protection posture, run the offline `MaxMind` provider with a **DB-IP
(EU) `.mmdb`** and register no online provider. The lookup happens entirely in-process, so the
client IP — personal data — **never leaves the application**: no HTTP call, no third-party
processor, no cross-border transfer to reason about under Schrems II / SCCs. Treat the US-hosted
ipinfo HTTP provider as an **opt-in online fallback**, added only when you accept that transfer.
</Aside>

</TabItem>
<TabItem label="ipinfo (online)">
...
Run MaxMind first (fast, offline), fall through to ipinfo only when the local database has
no answer.
