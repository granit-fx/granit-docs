---
title: "BFF Configuration Reference"
description: Reference for Granit.Bff — GranitBffOptions, YARP routes and clusters, OpenTelemetry metrics, distributed tracing, and per-env overrides.
sidebar:
  order: 6
  label: Configuration
---
...
| `ClientSide` | `MultiTenancySides?` | `null` | Side this frontend is restricted to (`Host` / `Tenant` / `Both`). When set, the BFF validates that the OpenIddict client carries the matching `client_side` setting; seed via `OidcApplicationSeedDescriptor.ClientSide` or `OpenIddictApplicationDescriptor.SetClientSide(...)`. Promoted to `Granit` foundation in PR #1068 so frontend scoping shares the same `MultiTenancySides` enum used by permission definitions — see [Multi-tenancy side](/dotnet/security/authorization-multitenancy-side/). |
...
|-------------|------|------|-------------|
| `granit.bff.session.login` | Counter | `tenant_id` | Successful BFF logins (callback completed) |
| `granit.bff.session.logout` | Counter | `tenant_id` | BFF logouts (session cleared) |
