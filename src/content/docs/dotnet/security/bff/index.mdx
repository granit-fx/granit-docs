...
description: Eliminate SPA token exposure with the BFF pattern — tokens stay server-side, the browser gets an HTTP-only cookie, APIs proxied via a confidential OIDC client.
sidebar:
  order: 1
  label: Overview
---
...
:::tip[Session enrichment]
Listing and revoking sessions runs through the canonical
[`/sessions` + `/devices` API](/dotnet/security/identity/#user-sessions--devices), backed by
`BffUserSessionProvider` when the BFF is configured. Each session carries a
[geolocation](/dotnet/security/ip-geolocation/) and a [risk level](/dotnet/security/user-sessions/),
letting a SPA flag a suspicious login ("new country, high risk") and offer one-click termination.
:::
...
- [Blog: Why your React app should never touch an access token](/blog/why-your-react-app-should-never-touch-an-access-token/) — the BFF pattern rationale in plain English
