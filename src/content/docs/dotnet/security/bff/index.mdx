...
| `GET` | `/bff/sessions` | List the user's active sessions, enriched with geolocation, risk level, and last-activity | Yes (session cookie) |
...
The `/bff/sessions` payload carries a [geolocation](/dotnet/security/ip-geolocation/) and a
[risk level](/dotnet/security/user-sessions/) per session, letting a SPA flag a suspicious
login ("new country, high risk") and offer one-click termination.
:::
