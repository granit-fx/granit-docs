---
title: "Security"
description: Deep dive into BFF security — cookie hardening, CSRF double-submit, clickjacking prevention, token storage encryption, sessions, and attack mitigation.
sidebar:
  order: 4
  label: Security
---
...
1. **User self-service** -- the canonical [`/sessions` API](/dotnet/security/identity/#user-sessions--devices)
   (backed by `BffUserSessionProvider` when the BFF is configured), not a BFF-specific route:
   - `GET /sessions` -- list active sessions, enriched with location and risk
   - `DELETE /sessions/{sessionId}` -- revoke a specific session ("log out that device")
   - `DELETE /sessions` -- revoke all other sessions ("log out everywhere else")
