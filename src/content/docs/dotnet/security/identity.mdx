...
public class AppModule : GranitModule
{
    ...
    {
        // The federated cache owns its own DbContext — pass the EF provider,
        // not a host context type.
        context.Builder.AddGranitIdentityFederatedEntityFrameworkCore(
            opts => opts.UseNpgsql(context.Configuration.GetConnectionString("Identity")));
...
Device trust, session risk, the behavioural profile, and single-use session reviews
share one contract — `IIdentitySecurityStateStore`. `Granit.Identity` registers
`MemoryIdentitySecurityStateStore` (in-memory, single-node — fine for development);
installing `Granit.Identity.EntityFrameworkCore` swaps in `EfCoreIdentitySecurityStateStore`,
mapped into the **same `IdentityDbContext`** — one DbContext, one set of migrations, no
separate database to provision. The application owns the migration.

```csharp
public interface IIdentitySecurityStateStore
{
    // Device trust
    Task SetDeviceTrustAsync(string userId, string deviceId, DeviceTrustVerdict verdict, CancellationToken ct = default);
    Task<DeviceTrustVerdict?> GetDeviceTrustAsync(string userId, string deviceId, CancellationToken ct = default);
    ...
    Task RevokeDeviceTrustAsync(string userId, string deviceId, CancellationToken ct = default);

    // Session risk, behavioural profile, and session review live on the same store.
}
```

:::note[Merged from four micro-stores]
`IDeviceTrustStore`, `IUserSessionRiskStore`, and their two siblings were merged into
`IIdentitySecurityStateStore`. Hosts that injected the old interfaces switch to the
merged one and prefix the method names with the concern (`SetDeviceTrustAsync`,
`GetSessionRiskAsync`, …).
:::
...
sequenceDiagram
    ...
    API->>Store: SetDeviceTrustAsync(user, deviceId, Remembered)
...
|--------|-------|------------|-------------|
| GET | `/identity/users` | `Identity.Users.Read` | Search cached users (free-text, paginated) |
| GET | `/identity/users/{userId}` | `Identity.Users.Read` | Get by external ID |
| POST | `/identity/users/batch` | `Identity.Users.Read` | Batch resolve by IDs |
| POST | `/identity/users/sync` | `Identity.Users.Sync` | Sync specific user |
| POST | `/identity/users/sync-all` | `Identity.Users.Sync` | Full sync from provider |
| DELETE | `/identity/users/{userId}` | `Identity.Users.Delete` | GDPR erase |
| PATCH | `/identity/users/{userId}/pseudonymize` | `Identity.Users.Delete` | GDPR pseudonymize |
| GET | `/identity/users/stats` | `Identity.Users.Read` | Cache statistics |
| GET | `/identity/users/capabilities` | `Identity.Users.Read` | Provider capabilities |
...
Both operations emit Wolverine integration events (`IdentityUserDeletedEto`,
`IdentityUserProfileUpdatedEto`) for downstream modules to react.
...
| Device trust | `IIdentitySecurityStateStore`, `DeviceTrustVerdict`, `DeviceTrustLevel`, `IDeviceTrustCookieService`, `DeviceTrustOptions`, `UseGranitDeviceTrust()` | `Granit.Identity` · `Granit.Identity.Endpoints` |
