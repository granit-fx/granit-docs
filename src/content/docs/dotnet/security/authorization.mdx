...
description: Dynamic RBAC with a runtime policy provider, per-role caching, EF Core grant store, and multi-tenant permission scoping — change roles without redeployment.
sidebar:
  order: 3
  label: Authorization
---
...
|---------|------|------------|
| `Granit.Authorization` | RBAC permissions, dynamic policy provider | `Granit`, `Granit.Caching` |
...
```mermaid
graph TD
    AZ[Granit.Authorization] --> S[Granit]
...
4. Multi-provider grant lookup — every grant carries a `(providerName, providerKey)` pair
   (`User`/`Role`/`Client`/`UserGroup`/…). Cache key:
   `perm:{tenantId|"global"}:{providerName}:{providerKey}:{permissionName}` — PR #1899
   collapses the lookup to a single per-(user, tenant, action) evaluator covering every
   provider in one round-trip.
5. `IPermissionGrantStore` fallback (default: `NullPermissionGrantStore` — always denied)

See [Multi-tenancy side](/dotnet/security/authorization-multitenancy-side/) for the full
provider matrix and the canonical `PermissionGrantProviderNames` constants.
...
Granit authorizes on **permissions, end to end — never on roles**. Endpoints gate on a
permission (`[Permission(...)]` or `RequireAuthorization(<permission>)`); roles exist only
as a way to *grant* permissions, never as a thing an endpoint checks directly. `RequireRole`
is banned everywhere — production code **and** tests.

```csharp
// ✅ Gate on the permission
group.MapDelete("/{userId}/sessions/{sessionId}", TerminateSession)
    .RequireAuthorization(IdentityPermissions.Sessions.Manage);   // "Identity.Sessions.Manage"

// ❌ Never gate on a role
group.MapDelete("/{userId}/sessions/{sessionId}", TerminateSession)
    .RequireAuthorization(p => p.RequireRole("admin"));           // forbidden
```

The reason is subtle but important: **a role-based policy in a test gives false confidence**.
It passes while exercising a mechanism production never uses, so the test proves nothing
about the permission the endpoint actually enforces. Tests must therefore grant the same
*permission* the endpoint requires — see [Testing authorization](#testing-authorization).

:::note[Enforced by an architecture test]
`EndpointAuthorizationRules.RequireRoleMustNotBeUsed` scans `src` **and** `tests` and fails
the build on any `RequireRole(...)` call. A companion rule
(`EveryEndpointMustDeclareAuthorizationStance`) requires every endpoint file to declare
`.RequireAuthorization(...)` or `.AllowAnonymous()` explicitly — there is no implicit default.
:::
...
```csharp
public class PermissionGrant : AuditedAggregateRoot, IMultiTenant
{
    public string Name { get; init; } = string.Empty;          // Permission name (immutable)
    public string ProviderName { get; init; } = string.Empty;  // User / Role / Client / UserGroup / …
    public string ProviderKey { get; init; } = string.Empty;   // userId, roleName, clientId, etc.
    public Guid? TenantId { get; set; }                        // Tenant scope (null = host/global)
}
// Unique index: (TenantId, ProviderName, ProviderKey, Name)
```

The shift from a role-only `RoleName` column to the `(ProviderName, ProviderKey)` pair
landed with ABP-style multi-provider grants in PR #1056 / #1899 — see the canonical
constants in `PermissionGrantProviderNames` (`User`, `Role`, `Client`, `UserGroup`).
...
{
    public async Task GrantToRoleAsync(
    ...
    {
        return await reader.GetGrantedPermissionsAsync(
            providerName: PermissionGrantProviderNames.Role,
            providerKey:  roleName,
            tenantId:     null,
            cancellationToken)
...
| Abstractions | `IPermissionDefinitionProvider`, `IPermissionDefinitionContext`, `IPermissionDefinitionRegistry`, `IPermissionChecker`, `IPermissionGrantStore`, `PermissionAttribute` | `Granit.Authorization` |
...
For **endpoint integration tests**, grant the exact *permission* the endpoint requires —
never a role (see [permission-based convention](#authorization-is-permission-based)). The
test auth handler reads a comma-separated `X-Test-Permissions` header and emits each entry as
a `permission` claim, which the endpoint's permission policy matches via
`RequireClaim("permission", <name>)`:

```csharp
// Grant exactly what the endpoint gates on — here Identity.Sessions.Manage
client.DefaultRequestHeaders.Add(
    TestAuthHandler.PermissionsHeader,            // "X-Test-Permissions"
    "Identity.Sessions.Read,Identity.Sessions.Manage");

var response = await client.DeleteAsync($"/identity/provider/users/{userId}/sessions/{sessionId}");
response.StatusCode.ShouldBe(HttpStatusCode.NoContent);
```

This keeps the test honest: it passes only because the caller holds the permission the
endpoint actually enforces, exactly as production does.
