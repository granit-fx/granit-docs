...
description: Tag permissions as Host, Tenant, or Both and grant them to roles, users, or OIDC clients. The checker enforces scope with specific-wins-over-generic order.
sidebar:
  order: 4
...
The `MultiTenancySides` property on the definition is consulted by
...
`MultiTenancySides` is not only a permission property — it is also a property of the
...
- [Authorization — Policies & RBAC](/dotnet/security/authorization/) — permission checker, evaluation order
- [Roles — RoleMetadata](/dotnet/security/authorization-roles/) — `RoleMetadata` aggregate and CRUD
- [Identity providers](/dotnet/security/identity/) — Keycloak, Entra, Cognito grant sources
- [Multi-tenancy](/dotnet/infrastructure/multi-tenancy/) — Host/Tenant boundary referenced by permissions
- [Inter-service authorization](/dotnet/security/inter-service-authorization/) — server-to-server permission propagation
