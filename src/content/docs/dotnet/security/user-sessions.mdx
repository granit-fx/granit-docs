...
<FileTree>
- Granit.Identity.Abstractions/ `UserSessionDescriptor`, `UserSessionRiskAssessment`, `IUserSessionAnomalyDetector`, `IUserSessionRiskStore`, the `...Eto` integration event — safe no-op defaults
- Granit.Identity.AnomalyDetection/ Heuristics (impossible travel, new device, new country) + opt-in AI layer over `Granit.AI`
- Granit.Identity.EntityFrameworkCore/ `IUserSessionRiskStore` backed by EF Core, persisted in the User DbContext (`user_sessions_risks`)
</FileTree>
...
the `user_sessions_risks` table. The table is mapped into the **User DbContext**
...
now lives in `Granit.Identity.EntityFrameworkCore`, and its `user_sessions_risks` table is part
of the User DbContext. Migrate by deleting the separate registration call and the
`UserSessions` connection string; once you call `AddGranitIdentityEntityFrameworkCore`, the risk
store is wired automatically. Existing `user_sessions_risks` data is unaffected — only its
owning DbContext changes.
</Aside>
...
| AI + heuristics | `IUserSessionRiskEvaluator`, `UserSessionsAnomalyDetectionOptions` | `Granit.Identity.AnomalyDetection` |
| Persistence | EF Core `IUserSessionRiskStore`, `GranitUserSessionsDbProperties` | `Granit.Identity.EntityFrameworkCore` |
