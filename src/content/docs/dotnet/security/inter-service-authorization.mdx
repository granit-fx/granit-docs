...
description: Propagate caller identity between Granit services — Client Credentials + DPoP, OAuth 2.0 Token Exchange (RFC 8693), and why bearer forwarding fails.
sidebar:
  order: 7
...
| Metric | Tags | Meaning |
|--------|------|---------|
| `granit.oidc.token_management.request.sent` | `tenant_id`, `grant_type`, `client_id` | Token endpoint calls (includes token-exchange grants) |
| `granit.oidc.token_management.cache.hit` | `tenant_id`, `client_name` | Cached-token reuse |
| `granit.oidc.token_management.cache.miss` | `tenant_id`, `client_name` | Forced IdP round-trip |
| `granit.oidc.token_management.error.occurred` | `tenant_id`, `error` | Any token endpoint refusal |
...
`Granit.Http.Resilience` used to ship `AddAuthTokenPropagation()` — a
`DelegatingHandler` that copied the inbound `Authorization` header verbatim
onto every outbound request. **This extension has been removed.**
