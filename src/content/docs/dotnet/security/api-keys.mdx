...
description: Long-lived API keys for service-to-service auth — hashed at rest with typed prefixes, CIDR allow-listing, lifecycle notifications, and daily expiration scan.
sidebar:
  order: 8
  label: API Keys
---
...
|---------|------|------------|
| `Granit.Authentication.ApiKeys` | `ApiKeyEntry` aggregate, `IApiKeyStore` / `IApiKeyAdminStore` / `IApiKeyGenerator`, `ApiKeyAuthenticationHandler`, CIDR validator | `Granit`, `Granit.Guids`, `Granit.Timing` |
...
|----------|---------|---------|-------------|
| `ExpirationLeadTimeDays` | `Authentication:ApiKeys` | `14` | Days before `ExpiresAt` at which the scanner starts emitting `ApiKeyExpiringSoonEto`. Range: 1–90. |
| `RoutePrefix` | `Authentication:ApiKeys:Endpoints` | `"authentication"` | URL prefix for the admin endpoints (full path: `/{prefix}/api-keys`). |
| `TagName` | `Authentication:ApiKeys:Endpoints` | `"API Keys"` | OpenAPI tag (Title Case With Spaces — see [tag conventions](/dotnet/architecture/http-conventions/#openapi-endpoint-metadata)). |
