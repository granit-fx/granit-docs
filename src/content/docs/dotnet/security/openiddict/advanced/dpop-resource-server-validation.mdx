...
description: "Validate DPoP proofs on any resource server — Keycloak, Entra ID, Auth0, or any OIDC provider. IdP-agnostic via Granit.Authentication.DPoP (RFC 9449 §7)."
sidebar:
  order: 6
...
```csharp
// No extra package, no configuration flag.
builder.AddGranitOpenIddictAuthentication();
```

The extension takes no delegate: it binds `GranitOpenIddictValidationOptions`
from the `Authentication:OpenIddict` section and always registers
`DPoPValidationTokenExtractionHandler`.

OpenIddict has **no DPoP support of its own** — its built-in
`ExtractAccessTokenFromAuthorizationHeader` only understands the `Bearer`
scheme. Granit adds the `DPoP`-scheme extractor (RFC 9449 §7.1) as a fallback
one slot after the Bearer extractor, so a resource server can authenticate
`Authorization: DPoP <token>` requests. It is harmless when no DPoP client is
in play, which is why there is nothing to switch on. Only works when the
...
|--------|------|------|-------------|
| `granit.authentication.dpop.validation.success` | Counter | `tenant_id` | Proof validations that passed |
| `granit.authentication.dpop.validation.failure` | Counter | `tenant_id`, `reason` | Proof validations that failed |
| `granit.authentication.dpop.replay.detected` | Counter | `tenant_id` | Replay attempts blocked by `jti` check |
