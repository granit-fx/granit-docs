...
|--------|-------|---------------|
| `LocalIdentity` | `openiddict_users` | Yes |
...
sequenceDiagram
    ...
    API->>Audit: Write durable audit entry (ImpersonationAuditMarker)
    Audit->>Events: Publish AuditEntryPersistedEto
...
| Transparency | The durable audit entry raises `AuditEntryPersistedEto`; `ImpersonationAuditNotificationHandler` consumes it and notifies the impersonated user (GDPR / SOC2) |
...
This is a **setting**, not an appsettings section — write it through
`ISettingWriter` so it can differ per tenant:

```csharp
await settingWriter.SetForTenantAsync(
    tenantId,
    OpenIddictSettingNames.IdleSessionTimeout,  // "OpenIddict.IdleSessionTimeout"
    "30",
    cancellationToken);
```
...
Passkey options are owned by `Granit.Identity.Local`
(`GranitPasskeyOptions`), bound from `Identity:Local:Passkey`:
...
### AuditEntryPersistedEto (impersonation)

Impersonation does not publish a dedicated event. The endpoint writes a
**durable audit entry** first, and the transparency notification is derived from
that record — so a notification can never exist without its audit trail.
...
**Published by:** the auditing pipeline, once the entry is committed.

**Impersonation subscriber:** `ImpersonationAuditNotificationHandler` filters on
the impersonation category (`ImpersonationAuditMarker`) and sends the
transparency notification to the impersonated user — GDPR / SOC2 compliance.
...
- [OIDC server configuration](/dotnet/security/openiddict/openid-connect-server/)
- [Account API](/dotnet/security/openiddict/account-management-api/)
- [Configuration reference](/dotnet/security/openiddict/configuration/)
- [Pushed Authorization Requests (PAR)](/dotnet/security/openiddict/advanced/pushed-authorization-requests/)
- [DPoP — Proof-of-Possession](/dotnet/security/openiddict/advanced/proof-of-possession-dpop/)
- [Private Key JWT Authentication](/dotnet/security/openiddict/advanced/private-key-jwt-authentication/)
- [FAPI 2.0 Security Profile](/dotnet/security/openiddict/advanced/fapi-2-security-profile/)
- [DPoP Resource Server Validation](/dotnet/security/openiddict/advanced/dpop-resource-server-validation/)
