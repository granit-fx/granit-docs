---
title: "OpenIddict Configuration Reference"
description: Reference for Granit.OpenIddict options — server, client, seeding, passkeys, per-tenant settings, feature flags, and permissions.
sidebar:
  order: 6
  label: Configuration
---
...
Social / external login is owned by `Granit.Authentication.External`
(`ExternalAuthOptions`), bound from `Authentication:External` — not from a
`Client` node under `OpenIddict`:
...
**Configuration section:** `Authentication:External`

**Package:** `Granit.Authentication.External`

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `AutoRegisterExternalUsers` | `bool` | `true` | Auto-create a `LocalIdentity` on first login via an external provider. When `false`, returns 403 if no existing account matches the email. |
| `Providers` | `ExternalAuthProvider[]` | `[]` | External login providers to configure. |
...
**Configuration section:** `Identity:Local:Passkey`

**Package:** `Granit.Identity.Local`
...
| `ServerName` | `string` | `"Granit"` | WebAuthn Relying Party display name. |
| `AllowedOrigins` | `string[]` | `[]` | Origins accepted during the WebAuthn ceremony. |
| `ChallengeLifetime` | `TimeSpan` | `00:05:00` | How long an issued challenge stays valid. |
...
These values are read from `GranitLockoutOptions` (section `Identity:Local:Lockout`).
...
backoff** via `LocalIdentityManager`. Each consecutive lockout doubles the duration,
...
- [User management](/dotnet/security/openiddict/user-management/)
