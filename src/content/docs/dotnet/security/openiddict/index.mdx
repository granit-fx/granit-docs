...
description: Turn any Granit app into a self-hosted OpenID Connect authorization server with user management, account self-service, and OIDC administration.
sidebar:
  order: 1
  label: Overview
---
...
- Granit.OpenIddict/ Abstractions, entities, options, permissions, events
  - Granit.OpenIddict.Server/ Token, authorization, introspection, revocation, userinfo endpoints (`AddGranitOpenIddictServer`)
  - Granit.OpenIddict.EntityFrameworkCore/ DbContext, application/authorization/scope stores, seeding
...
| `Granit.OpenIddict.Server` | Token + authorization + introspection + revocation + userinfo endpoints (`AddGranitOpenIddictServer()`) |
| `Granit.OpenIddict.EntityFrameworkCore` | `OpenIddictDbContext`, EF-backed application / authorization / scope stores, seed contributor, host builder extension |
...
graph TD
    ...
    OID --> EVT[Granit.Events]
    SRV[Granit.OpenIddict.Server] --> OID
    OID --> SEC[Granit]
...
### No ISoftDeletable on LocalIdentity

`LocalIdentity` extends `IdentityUser<Guid>` which is managed by ASP.NET Core Identity's
`UserManager<T>`. The `UserManager` uses reflection and metadata patterns that are
incompatible with the `ISoftDeletable` interface. Instead, `LocalIdentity` has manual
...
`LocalIdentity` is the source of truth — `UserCacheEntry` and `UserCacheSyncMiddleware` are
...
| `AuditEntryPersistedEto` (impersonation marker) | Admin impersonation | Transparency notification, audit log |

## Next steps

- [OIDC Server Configuration](/dotnet/security/openiddict/openid-connect-server/) -- flows, endpoints, signing keys
- [User Management](/dotnet/security/openiddict/user-management/) -- entities, admin API, IIdentityProvider bridge
- [Account Self-service API](/dotnet/security/openiddict/account-management-api/) -- registration, 2FA, GDPR
- [Advanced Features](/dotnet/security/openiddict/advanced/) -- multi-tenancy, impersonation, passkeys, idle sessions
