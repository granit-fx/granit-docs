---
title: "Granit Bundles — start a new project with one PackageReference, not thirty"
description: "Meta-packages that group Granit modules by job — Essentials, Api, OpenIddict, Notifications (granit-dotnet), and Documents + SaaS (granit-business). One PackageReference, one Add*() call, and the whole feature group is wired correctly. Skip the bundle and reference modules directly when you need fine-grained control."
sidebar:
  label: Bundles
  order: 10
---
...
| `Granit.Http.SecurityHeaders` | CSP, HSTS, X-Content-Type-Options |
...
- [Framework vs modules](./framework-vs-modules/) — what lives in `granit-dotnet` vs `granit-business`
