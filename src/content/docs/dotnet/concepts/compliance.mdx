---
title: "Compliance as Architecture — GDPR & ISO 27001 Built In"
description: How Granit embeds GDPR (Art. 15, 17, 32) and ISO 27001 A.12.4 as architectural constraints — audit trails, transparent encryption, and tenant isolation.
sidebar:
  label: Compliance
  order: 9
---
...
## Where to start, by regulation

| You are targeting | Start here |
|---|---|
| **GDPR** — erasure, portability, restriction, privacy by design | This page, then [Crypto-Shredding](/dotnet/compliance/crypto-shredding/) (cryptographic erasure) and [Privacy](/dotnet/compliance/privacy/) (data export API) |
| **ePrivacy / cookie consent** (incl. CCPA-style regimes) | [Cookies](/dotnet/compliance/cookies/) — consent ledger, CMP integration, regulation bridge |
| **ISO 27001** | This page, then [Audit Log](/dotnet/compliance/audit-log/) and [Observability](/dotnet/operations/observability/) |
| **NIS2** | [NIS2-ready .NET applications](/blog/nis2-ready-dotnet-applications/) walkthrough, plus [Audit Log](/dotnet/compliance/audit-log/) |
| **SOC 2 Type II** | [SOC 2 Type II-ready SaaS with Granit](/blog/soc2-type2-ready-saas-granit/) walkthrough |
...
When retention rules require the data itself to become unrecoverable — not just
hidden — [Crypto-Shredding](/dotnet/compliance/crypto-shredding/) complements soft
delete: destroy the per-entity encryption key and the encrypted columns are
permanently unreadable while rows and audit references stay intact.
...
`Granit.Privacy` provides the `IPrivacyDataProvider` interface (namespace
`Granit.Privacy.DataExport`). Each module that stores personal data registers a
provider that streams its data as export fragments.

```csharp
public sealed class PatientPrivacyDataProvider(
    IPatientReader patients,
    IStagedFragmentBuilder fragmentBuilder) : IPrivacyDataProvider
{
    public static string ProviderName => "medical-records";
    public static string DisplayKey => "Privacy.Scopes.MedicalRecords";
    public static string? FeatureName => null;

    public async ValueTask<bool> HasDataAsync(
        PrivacyExportContext context, CancellationToken cancellationToken)
        => await patients.ExistsAsync(context.SubjectUserId, cancellationToken);

    public async IAsyncEnumerable<ExportFragment> ExportAsync(
        PrivacyExportContext context,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        // Stage each record as a fragment; the archive assembler picks them up.
        yield return await fragmentBuilder.BuildJsonAsync(
            context,
            ProviderName,
            "medical-records/patient.json",
            await patients.GetAsync(context.SubjectUserId, cancellationToken),
            cancellationToken);
    }
}
```

The static abstract members (`ProviderName`, `DisplayKey`, `FeatureName`) let the
privacy module enumerate and localize export scopes without instantiating a
provider. `FeatureName` gates the scope behind a feature flag when non-null.

Fragments are streamed rather than buffered: `StagedExportFragment` points at a
blob the provider wrote, `PassThroughExportFragment` references bytes that
already live in blob storage. The privacy module aggregates every registered
provider into one signed archive — satisfying the right to data portability
...
|----------------|---------------|----------|
| `AesStringEncryptionProvider` | Local config key (AES-256-CBC) | Development, testing |
| `HashiCorpTransitEncryptionService` | HashiCorp Vault Transit | Production (key never leaves Vault) |
...
- [Audit Log](/dotnet/compliance/audit-log/) -- DB-backed audit entries with per-entity change snapshots
- [Crypto-Shredding](/dotnet/compliance/crypto-shredding/) -- cryptographic erasure for GDPR Art. 17
- [Cookies](/dotnet/compliance/cookies/) -- consent management and the multi-regulation bridge
...
- [Security model concept](/dotnet/concepts/security-model/) -- authentication and authorization architecture
- [Multi-tenancy concept](/dotnet/concepts/multi-tenancy/) -- tenant resolution and isolation strategies
