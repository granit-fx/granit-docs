...
description: Use Granit with or without Wolverine — Channel<T> handles in-process events for free, Wolverine adds distributed, guaranteed delivery when you need to graduate.
sidebar:
...
Five modules ship with both a Wolverine integration package and an in-process
default (most use `Channel<T>`, scheduling uses a Cronos-backed ticker):
...
Each base module registers its in-process default. When the corresponding Wolverine
package is added and its module is declared in `[DependsOn]`, it **replaces** the
default with a durable outbox-backed implementation. No code changes in your
handlers.

### How Channel dispatch works

The pattern is the same across the `Channel<T>` modules. Taking background jobs as an example:
...
Once Wolverine is running in production, switch to **Static** code generation so cold
starts no longer recompile every handler with Roslyn. See [Production code
generation](/dotnet/infrastructure/wolverine-messaging/#production-code-generation) for
the pre-generation recipe.
:::
...
- [Messaging](/dotnet/concepts/messaging/) -- domain events, integration events, transactional outbox, context propagation
