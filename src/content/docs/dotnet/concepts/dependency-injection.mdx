...
description: How Granit modules self-register services with [DependsOn] ordering, the Options pattern, service lifetimes, and no separate Abstractions packages.
sidebar:
...
{
    public override void ConfigureServices(ServiceConfigurationContext context) =>
        context.Builder.AddGranitSettingsEntityFrameworkCore();
}
```
...
- [Module System](/dotnet/concepts/module-system/) — how `[DependsOn]` and the two-phase lifecycle work
- [Configuration](/dotnet/concepts/configuration/) — Options vs Settings vs Module Config decision tree
- [Security Model](/dotnet/concepts/security-model/) — PostConfigure pattern in action (Keycloak overriding JwtBearer)
