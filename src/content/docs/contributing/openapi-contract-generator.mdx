...
### Wolverine types must resolve during the assembly scan

`Granit.Privacy` (a base module) declares Wolverine sagas, and it marks `WolverineFx`
with `PrivateAssets="all"` so `Wolverine.dll` does **not** flow transitively to
consumers. Composing the module graph reflects over Privacy's exported types, which
references Wolverine — so the type scan must tolerate the dependency being absent. The
framework's assembly scan was hardened to do exactly that, which is what lets the
generator compose `Granit.Privacy.Endpoints` without dragging Wolverine into every
consumer.

<Aside type="note">
A live working-tree change in `granit-dotnet` is removing the generator's private
`WolverineFx` reference now that the scan tolerates the absent assembly. Confirm
against the merged state of PR #2542 before relying on the exact `.csproj` package
list — the **behavior** (Privacy composes without Wolverine flowing transitively) is
the stable contract here.
</Aside>
